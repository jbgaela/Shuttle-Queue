...
export const api = {
  ...
  createSuggestedMatch: (_workspaceId: string, body: { teamA: string[]; teamB: string[]; suggestionToken: string; suggestionAdjusted?: boolean; courtId?: string }) => request<Match>("/matches", { method: "POST", body: JSON.stringify(body) }),
  updateMatch: (match: Match, body: { teamA: string[]; teamB: string[] }) => request<Match>(`/matches/${match.id}`, { method: "PATCH", ...(match.version === undefined ? {} : { headers: { "if-match": String(match.version) } }), body: JSON.stringify(body) }),
...
export type QueueState = { inactive: SessionPlayer[]; waiting: SessionPlayer[]; queued: SessionPlayer[]; playing: SessionPlayer[]; resting: SessionPlayer[]; serverTime: string; minimumRestMinutes?: number; lateArrivalCutoffAt?: string | null };
...
export type Suggestion = { token: string; expiresAt: number; key: string; strengthGap?: 1 | 2 | 3; difference: number; teamATotal: number; teamBTotal: number; lateArrivalCutoffAt?: string | null; teamA: SessionPlayer[]; teamB: SessionPlayer[]; explanation: { repeatPenalties?: Record<string, number>; rest?: { minimumRestMinutes?: number; eligibleAt?: string }; lateArrival?: { minimumPending?: number; selectedPending?: number; preferenceApplied?: boolean }; skillDiversity?: { groupSpread?: number; partnerMix?: number }; fairness?: { minimumGames?: number; minimumGamesCount?: number; manualOverride?: boolean; previouslySkippedCount?: number }; partnerRotation?: { recentRepeats?: number; allTimeRepeats?: number; preservedTeamBalance?: boolean }; algorithmVersion?: string; cycleRestarted?: boolean } };
export type Match = { id: string; status: string; source?: string; courtId?: string | null; matchmakingMode?: string | null; algorithmVersion?: string | null; suggestionKey?: string | null; suggestionExplanation?: unknown; startedAt?: string | null; completedAt?: string | null; queuedAt?: string; winnerTeam?: string | null; cancellationReason?: string | null; version?: number; scoring?: { pointsToWin: number; winBy: number; scoreCap: number | null; bestOf: number }; participants: { id?: string; queuePlayerId?: string; displayName?: string; playerStatus?: string; lastMatchEndedAt?: string | null; team: string; teamSlot?: number }[] };
...
export type PublicRankingPayload = { sessionStartedAt: string; firstMatchStartedAt: string | null; sessionEndedAt?: string | null; state: "LIVE" | "FINAL"; serverTime: string; lastUpdatedAt: string; historyAvailable: boolean; rankings: PublicRankingRow[] };
