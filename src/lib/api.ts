...
export type QueuePlayer = { id: string; playerId?: string; displayName: string; gender: string; skillLevel: string; skillWeight: number; status: string; matchesPlayed: number; wins: number; losses: number; pointsFor?: number; pointsAgainst?: number; amountDueMinor?: number; manualPriority?: number; queueEnteredAt?: string | null; checkedInAt?: string | null; checkedOutAt?: string | null; currentMatchId?: string | null; latePenaltyState: "PENDING" | "SERVED" | "WAIVED" | null; latePenaltyAppliedAt?: string | null; version?: number };
...
export type Match = { id: string; status: string; source?: string; courtId?: string | null; startedAt?: string | null; completedAt?: string | null; queuedAt?: string; winnerTeam?: string | null; cancellationReason?: string | null; version?: number; scoring?: { pointsToWin: number; winBy: number; scoreCap: number | null; bestOf: number }; participants: { id?: string; queuePlayerId?: string; displayName?: string; playerStatus?: string; team: string; teamSlot?: number }[] };
