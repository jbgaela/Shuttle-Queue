...
export const api = {
  ...
  logout: async () => { try { if (typeof navigator !== "undefined" && !navigator.onLine) return; await refreshCsrfToken(); return await request<void>("/auth/logout", { method: "POST" }); } finally { csrfTokenCache = null; if (typeof window !== "undefined") { try { window.sessionStorage.removeItem("bq-csrf-token"); } catch { /* ignore storage cleanup failures */ } } } },
  ...
  updateCourt: (_workspaceId: string, courtId: string, body: { name?: string; status?: "AVAILABLE" | "CLOSED" }) => request<Court>(`/courts/${courtId}`, { method: "PATCH", body: JSON.stringify(body) }),
...
export type Match = { id: string; status: string; source?: string; courtId?: string | null; startedAt?: string | null; completedAt?: string | null; queuedAt?: string; winnerTeam?: string | null; cancellationReason?: string | null; participants: { id?: string; queuePlayerId?: string; displayName?: string; team: string; teamSlot?: number }[] };
