...
export type ChallengeNoMatchReason = "NO_QUALIFIER" | "QUALIFIERS_UNAVAILABLE" | "INSUFFICIENT_READY_PLAYERS" | "INCOMPATIBLE_LINEUP_CONSTRAINTS" | "NO_ALLOWED_STRENGTH_DIFFERENCE" | "ALTERNATIVES_EXHAUSTED";

function emitChallengeNotifications<T extends { notifications?: UndefeatedChallengeNotification[] }>(value: T) {
  if (typeof window !== "undefined" && value.notifications?.length) {
    window.dispatchEvent(new CustomEvent("undefeated-challenge-eligible", { detail: value.notifications }));
  }
  return value;
}
...
export const api = {
  ...
  createSuggestedMatch: (_workspaceId: string, body: { teamA: string[]; teamB: string[]; suggestionToken: string; suggestionAdjusted?: boolean; overrideToManual?: boolean; playerPreferenceConfirmed?: boolean; courtId?: string }) => request<Match>("/matches", { method: "POST", body: JSON.stringify(body) }),
  updateMatch: (match: Match, body: { teamA: string[]; teamB: string[]; courtId?: string; swapWithMatchId?: string; overrideToManual?: boolean }) => request<Match>(`/matches/${match.id}`, { method: "PATCH", ...(match.version === undefined ? {} : { headers: { "if-match": String(match.version) } }), body: JSON.stringify(body) }),
  ...
  startSuggestedMatch: (_workspaceId: string, body: { teamA: string[]; teamB: string[]; courtId: string; suggestionToken: string; suggestionAdjusted?: boolean; overrideToManual?: boolean; playerPreferenceConfirmed?: boolean }) => request<Match>("/matches/start-suggestion", { method: "POST", body: JSON.stringify(body) }),
  completeMatch: async (matchId: string, games: { teamAScore: number; teamBScore: number }[]) => emitChallengeNotifications(await request<Match>(`/matches/${matchId}/complete`, { method: "POST", body: JSON.stringify({ games }) })),
  correctMatch: async (matchId: string, games: { teamAScore: number; teamBScore: number }[], reasonOrVersion?: string | number, versionOrReason?: number | string) => {
    const reason = typeof reasonOrVersion === "string" ? reasonOrVersion : typeof versionOrReason === "string" ? versionOrReason : undefined;
    const version = typeof reasonOrVersion === "number" ? reasonOrVersion : typeof versionOrReason === "number" ? versionOrReason : undefined;
    return emitChallengeNotifications(await request<Match>(`/matches/${matchId}/correct`, { method: "POST", ...(version === undefined ? {} : { headers: { "if-match": String(version) } }), body: JSON.stringify({ games, ...(reason ? { reason } : {}) }) }));
  },
  correctMatchParticipants: async (matchId: string, teamA: string[], teamB: string[], reason?: string, version?: number) => emitChallengeNotifications(await request<Match>(`/matches/${matchId}/correct-participants`, { method: "POST", ...(version === undefined ? {} : { headers: { "if-match": String(version) } }), body: JSON.stringify({ teamA, teamB, ...(reason ? { reason } : {}) }) })),
...
export type GuidedAvailability = { available: boolean; reason?: GuidedAvailabilityReason | null; waitingLearnerCount: number; waitingGuideCount: number; readyLearnerCount: number; readyGuideCount: number; nextEligibleAt?: string | null };
export type QueueState = { inactive: SessionPlayer[]; waiting: SessionPlayer[]; queued: SessionPlayer[]; playing: SessionPlayer[]; resting: SessionPlayer[]; serverTime: string; minimumRestMinutes?: number; lateArrivalCutoffAt?: string | null; synergyTeams?: SynergyTeam[]; guided?: GuidedAvailability; undefeatedChallenge?: { minimumMatches: 5; rankLimit: 3; players: UndefeatedChallengePlayer[] } };
...
export type Suggestion = { token: string; mode?: string; expiresAt: number; key: string; strengthGap?: 1 | 2 | 3; difference: number; teamATotal: number; teamBTotal: number; lateArrivalCutoffAt?: string | null; matchupAdvisory?: MatchupAdvisory | null; teamA: SessionPlayer[]; teamB: SessionPlayer[]; explanation: { strengthGap?: 1 | 2 | 3; guided?: { learnerSkillLevels?: string[]; guideSkillLevels?: string[]; learnerIds?: string[]; guideIds?: string[] }; challenge?: { selectedPlayerIds?: string[]; opposingPlayerIds?: string[]; qualifyingPlayers?: Array<{ id: string; displayName: string; rank: number; gamesPlayed: number; wins: number; losses: number }>; rotatedPair?: boolean; fallbackIncludedThird?: boolean; difficultyPolicy?: string; appliedDisadvantage?: 0 | 1 | 2 | null; equalStrengthFallback?: boolean }; searchStats?: { eligibleCount?: number; evaluatedCount?: number; bounded?: boolean }; repeatPenalties?: Record<string, number>; rest?: { minimumRestMinutes?: number; eligibleAt?: string }; lateArrival?: { minimumPending?: number; selectedPending?: number; preferenceApplied?: boolean }; skillDiversity?: { groupSpread?: number; partnerMix?: number }; fairness?: { minimumGames?: number; minimumGamesCount?: number; manualOverride?: boolean; previouslySkippedCount?: number; supportMinimumGames?: number; supportPending?: number }; partnerRotation?: { recentRepeats?: number; allTimeRepeats?: number; preservedTeamBalance?: boolean }; algorithmVersion?: string; cycleRestarted?: boolean } };
...
export type MatchProvenance = { kind: "MANUAL" | "GENERATED" | "ADJUSTED_SUGGESTION" | "LEGACY_ADJUSTED"; label: string; description: string; originalMode: string | null; guaranteesRetained: boolean | null };
export type HistoryMatch = { id: string; source: string; matchmakingMode?: string | null; matchmakingLabel?: string; provenance?: MatchProvenance; format: "SINGLES" | "DOUBLES"; court: { id: string; name: string } | null; startedAt?: string | null; completedAt?: string | null; durationSeconds: number | null; winnerTeam: "A" | "B" | null; version: number; scoring: { pointsToWin: number; winBy: number; scoreCap: number | null; bestOf: 1 | 3 }; score: { revisionNumber: number; winnerTeam: "A" | "B"; games: HistoryGame[] } | null; participants: HistoryParticipant[] };
