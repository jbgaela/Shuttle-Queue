...
export const api = {
  ...
  updateMatch: (match: Match, body: { teamA: string[]; teamB: string[]; courtId?: string }) => request<Match>(`/matches/${match.id}`, { method: "PATCH", ...(match.version === undefined ? {} : { headers: { "if-match": String(match.version) } }), body: JSON.stringify(body) }),
