...
export const api = {
  ...
  correctMatch: async (matchId: string, games: { teamAScore: number; teamBScore: number }[], reason?: string) => emitChallengeNotifications(await request<Match>(`/matches/${matchId}/correct`, { method: "POST", body: JSON.stringify({ games, ...(reason ? { reason } : {}) }) })),
...
export type HistoryMatch = { id: string; source: string; matchmakingMode?: string | null; matchmakingLabel?: string; format: "SINGLES" | "DOUBLES"; court: { id: string; name: string } | null; startedAt?: string | null; completedAt?: string | null; durationSeconds: number | null; winnerTeam: "A" | "B" | null; score: { revisionNumber: number; winnerTeam: "A" | "B"; games: HistoryGame[] } | null; participants: HistoryParticipant[] };
