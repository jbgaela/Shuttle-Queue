...
const challengeNoMatchDetails = (players: MatchPlayer[], snapshot: CloudSnapshotV2, excludedKeys: string[] = []) => {
  ...
  const ready = waiting.filter((player) => Date.parse(restEligibleAt(findQueuePlayer(snapshot, player.id)!, snapshot, nowValue)) <= nowValue);
  const readyQualifierCount = ranked.filter(({ player }) => player.status === "WAITING" && Date.parse(restEligibleAt(findQueuePlayer(snapshot, player.id)!, snapshot, nowValue)) <= nowValue).length;
  const reason = ranked.length === 0 ? "NO_QUALIFIER" : readyQualifierCount === 0 ? "QUALIFIERS_UNAVAILABLE" : ready.length < 4 ? "INSUFFICIENT_READY_PLAYERS" : excludedKeys.length > 0 ? "ALTERNATIVES_EXHAUSTED" : "INCOMPATIBLE_LINEUP_CONSTRAINTS";
  return { qualifierCount: ranked.length, readyQualifierCount, readyPlayerCount: ready.length, reason };
};
const challengePersistenceDetails = (teamA: MatchPlayer[], teamB: MatchPlayer[], selectedPlayerIds: string[]) => {
  const selected = new Set(selectedPlayerIds);
  const qualifierTeam = teamA.some((player) => selected.has(player.id)) ? teamA : teamB;
  const opponentTeam = qualifierTeam === teamA ? teamB : teamA;
  const appliedDisadvantage = selected.size === 1
    ...
    : null;
  if (!selected.size || (appliedDisadvantage !== null && ![0, 1, 2].includes(appliedDisadvantage))) throw new Error("UNDEFEATED_CHALLENGE_CONSTRAINT: The undefeated player no longer has a legal Challenge lineup. Generate a new challenge or continue as Manual Adjusted.");
  return { minimumMatches: UNDEFEATED_CHALLENGE_MINIMUM_MATCHES, rankLimit: UNDEFEATED_CHALLENGE_RANK_LIMIT, selectedPlayerIds, opposingPlayerIds: selected.size > 1 ? selectedPlayerIds : [], difficultyPolicy: selected.size === 1 ? "TEAM_TOTAL_PLUS_ONE_THEN_PLUS_TWO_THEN_EQUAL" : "QUALIFIED_PLAYERS_OPPOSED", appliedDisadvantage, equalStrengthFallback: selected.size === 1 && appliedDisadvantage === 0 };
};
...
async function makeSuggestion(accountId: string, body: Record<string, unknown>) {
  ...
  if (String(body.mode) === "UNDEFEATED_CHALLENGE" && result.noMatch?.message === "No valid Undefeated Challenge lineup is available under the challenge rules.") {
    return { ...result, noMatch: { ...result.noMatch, message: "No legal +1, +2, or equal-strength Undefeated Challenge lineup is available under the current queue constraints." } };
  }
  if (String(body.mode) !== "GUIDED") return result;
  const snapshot = await readSnapshot(accountId);
...
async function createMatchStacked(accountId: string, body: Record<string, unknown>) {
  return mutate(accountId, "MATCH_CREATED", (snapshot) => {
    ...
    if (suggestionPayload?.mode === "UNDEFEATED_CHALLENGE" && !overrideToManual && !challengeAdjusted && suggestionExplanation && typeof suggestionExplanation === "object") Object.assign(suggestionExplanation, { challenge: challengePersistenceDetails(teamAPlayers, teamBPlayers, challengeSelectedPlayerIds) });
