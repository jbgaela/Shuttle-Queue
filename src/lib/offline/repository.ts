...
async function makeSuggestion(accountId: string, body: Record<string, unknown>) {
  ...
  let suggestion = suggestMatch(input, mode, { partners: new Map(), opponents: new Map(), quartets: new Map() }, excludedKeys);
  let cycleRestarted = false;
  if (!suggestion && excludedKeys.length) {
    suggestion = suggestMatch(input, mode, { partners: new Map(), opponents: new Map(), quartets: new Map() });
    cycleRestarted = Boolean(suggestion);
  }
  if (!suggestion) {
    const message = mode === "BALANCED" ? "No balanced lineup is available with player skill and team strength gaps of 1 or less. Wait for more eligible players or use Manual mode." : "No eligible group satisfies this mode.";
    return { suggestion: null, cycleRestarted: false, noMatch: { code: "NO_VALID_GROUP", message } };
  }
  const convert = (player: MatchPlayer) => playerView(findQueuePlayer(snapshot, player.id)!);
