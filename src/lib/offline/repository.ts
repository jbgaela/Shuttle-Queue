...
const playerView = (player: DomainQueuePlayer): QueuePlayer => ({ id: player.id, playerId: player.playerId, displayName: player.displayName, gender: player.gender, skillLevel: player.skillLevel, skillWeight: player.skillWeight, status: player.status, matchesPlayed: player.matchesPlayed, wins: player.wins, losses: player.losses, pointsFor: player.pointsFor, pointsAgainst: player.pointsAgainst, amountDueMinor: player.amountDueMinor ?? 0, manualPriority: player.manualPriority ?? 0, queueEnteredAt: player.queueEnteredAt ?? null, checkedInAt: player.checkedInAt ?? null, checkedOutAt: player.checkedOutAt ?? null, currentMatchId: player.currentMatchId ?? null, latePenaltyState: player.latePenaltyState ?? null, latePenaltyAppliedAt: player.latePenaltyAppliedAt ?? null, version: player.version });
...
async function createMatchStacked(accountId: string, body: Record<string, unknown>) {
  return mutate(accountId, "MATCH_CREATED", (snapshot) => {
    ...
    const allowed = manualQueue ? ["WAITING", "QUEUED", "PLAYING"] : ["WAITING"];
...
async function endQueue(accountId: string) {
  return mutate(accountId, "WORKSPACE_ENDED", (snapshot) => {
    ...
    const allocations = config?.mode === "EQUAL_SPLIT" ? allocateEqualSplitOffline(checkedIn, config.expectedQueueCostMinor ?? 0) : new Map(checkedIn.map((player) => [player.id, config?.fixedAmountPerPlayerMinor ?? 0]));
    for (const player of checkedIn) player.amountDueMinor = allocations.get(player.id) ?? 0;
