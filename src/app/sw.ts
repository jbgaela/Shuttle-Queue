...
const runtimeCaching: RuntimeCaching[] = [
  { matcher: ({ sameOrigin, url: { pathname } }) => sameOrigin && pathname.startsWith("/rankings/"), handler: new NetworkOnly() },
...
const serwist = new Serwist({ precacheEntries: self.__SW_MANIFEST.filter((entry) => !(typeof entry === "string" ? entry : entry.url).startsWith("/rankings/")), skipWaiting: true, clientsClaim: true, runtimeCaching, fallbacks: { entries: [{ url: "/", matcher: ({ request }) => request.mode === "navigate" }] } });
