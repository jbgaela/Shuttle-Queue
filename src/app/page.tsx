...
import { DidNotPlaySection, NotYetEligibleSection } from "@/components/did-not-play";
...
function RankingsView({ sessionId, session }: { sessionId: string; session?: SessionSummary }) {
  ...
  return <div className="space-y-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-[var(--muted)]">Current queue rankings</p><h2 className="display text-4xl">LineDrive Afternoon Queue</h2></div><Button variant="quiet" className="self-start px-3 sm:self-auto" disabled={!rankings.length || saving || rankingsQuery.isError} loading={saving} onClick={() => void save()}><Download size={16} /> {saving ? "Saving…" : "Save to device"}</Button></div>{sessionForSharing && <RankingsSharingControls session={sessionForSharing} />}<Card className="overflow-hidden p-0"><div className="border-b border-[var(--line)] bg-[#fbfdfb] px-4 py-3 text-xs leading-5 text-[var(--muted)]">Prize rankings use a confidence-adjusted win score. Players need 5 completed games; the top 3 eligible players are prize positions.</div>{rankingsQuery.isPending ? <div className="p-6 text-sm text-[var(--muted)]">Loading rankings…</div> : rankingsQuery.isError ? <p role="alert" className="p-4 text-sm text-[#8d4824]">{errorMessage(rankingsQuery.error)}</p> : rankings.length === 0 ? <div className="p-4"><EmptyState icon={Trophy} title="No results yet" body="Finish a match and the leaderboard will appear here." /></div> : <><div>{ranked.map((row) => { const rankingId = row.queuePlayerId ?? row.sessionPlayerId; return <div key={rankingId} className="border-b border-[var(--line)] last:border-0"><button type="button" className={`focus-ring flex w-full items-center gap-4 px-4 py-4 text-left ${row.isPrizePosition ? "bg-[#fffaf0]" : ""}`} aria-expanded={expandedId === rankingId} onClick={() => setExpandedId((current) => current === rankingId ? null : rankingId)}><span className="grid size-9 place-items-center rounded-full bg-[#edf8f4] text-sm font-bold text-[var(--teal)]">{row.rank}</span><div className="min-w-0 flex-1"><p className="truncate font-semibold">{row.player}{row.isPrizePosition ? <span className="ml-2 text-xs font-bold uppercase tracking-[0.1em] text-[#a85b2b]">Prize</span> : null}</p><p className="text-xs text-[var(--muted)]">{row.gender === "MALE" ? "Male" : "Female"} · {pretty(row.skillLevel)} · {row.matchesPlayed} games · {row.wins}W / {row.losses}L{row.seededDrawUsed ? " · seeded draw tiebreak" : ""}</p></div><div className="text-right"><p className="font-semibold">{(row.winRateBasisPoints / 100).toFixed(0)}%</p><p className="text-xs text-[var(--muted)]">win rate · score {(row.rankingScoreBasisPoints ?? 0) / 100}%</p></div><ChevronDown size={18} className={`shrink-0 text-[var(--muted)] transition ${expandedId === rankingId ? "rotate-180" : ""}`} /></button>{expandedId === rankingId && <PlayerHistoryDetails sessionId={sessionId} ranking={row} />}</div>; })}</div><NotYetEligibleSection players={notYetEligible} /><DidNotPlaySection players={didNotPlay} /></>}</Card></div>;
}
