...
import { evaluateGuidedAvailability, lowSkillLoneFemaleAdvisory, validateGuidedLineup, validateMixedDoublesLineup, type GuidedLineupPlayer, type MatchPlayer as DomainMatchPlayer, type Gender as DomainGender, type QueuePlayerStatus as DomainQueuePlayerStatus, type SkillLevel as DomainSkillLevel } from "@/lib/offline/domain-compat";
...
const MATCHMAKING_MODE_OPTIONS = [
...
function MatchmakerPanel({ sessionId, queue, courts, onChanged, guidedRequestId: guidedRequestIdProp = 0, onGuidedPendingChange: onGuidedPendingChangeProp }: { sessionId: string; queue: QueueState; courts: Court[]; onChanged: () => void; guidedRequestId?: number; onGuidedPendingChange?: (pending: boolean) => void }) {
  ...
  const guidedDraftTeamA: GuidedLineupPlayer[] = draftTeamAPlayers.map((player) => ({ id: player.id, skillLevel: isDomainSkill(player.skillLevel) ? player.skillLevel : "NEWBIE" }));
  const guidedDraftTeamB: GuidedLineupPlayer[] = draftTeamBPlayers.map((player) => ({ id: player.id, skillLevel: isDomainSkill(player.skillLevel) ? player.skillLevel : "NEWBIE" }));
  const draftGuidedError = mode === "GUIDED" ? validateGuidedLineup(guidedDraftTeamA, guidedDraftTeamB) : null;
  ...
  const draftConstraintError = draftMixedError ?? draftGuidedError ?? draftGenderError ?? draftBalanceError;
  ...
  useEffect(() => {
    ...
    suggest.mutate(undefined, { onError: () => { requestFailed = true; }, onSettled: () => {
      onGuidedPendingChange?.(false);
