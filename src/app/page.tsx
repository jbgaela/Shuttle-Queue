...
function MatchupAdvisoryBanner({ advisory, compact = false }: { advisory: NonNullable<Match["matchupAdvisory"]>; compact?: boolean }) {
  return <div role={compact ? undefined : "alert"} className={`flex items-start gap-3 rounded-2xl border border-[#e9b27f] bg-[#fff8f1] px-3 py-3 text-[#7d482b] ${compact ? "border-l-4" : ""}`}>
    <AlertTriangle aria-hidden="true" className="mt-0.5 shrink-0 text-[#b96b32]" size={18} />
    <div className="min-w-0"><p className="text-sm font-bold">{compact ? "Player check needed" : "Player preference check"}</p><p className="mt-1 text-xs leading-5">{advisory.displayName} is the only female player in this matchup and is listed as {pretty(advisory.skillLevel)}. Check that she is comfortable playing against an all-male pair before starting.</p></div>
  </div>;
}
...
function MatchmakerPanel({ sessionId, queue, courts, onChanged }: { sessionId: string; queue: QueueState; courts: Court[]; onChanged: () => void }) {
  ...
  const matchupAdvisory = suggestion && draft ? lowSkillLoneFemaleAdvisory(draftTeamAPlayers, draftTeamBPlayers) : null;
  ...
  const renderSlot = (team: SuggestionTeam, slot: number) => {
    ...
    const content = <><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{player?.displayName ?? "Player unavailable"}</span><span className="block truncate text-xs text-[var(--muted)]">{player ? `${playerDetails(player)} · ${player.matchesPlayed} games` : "No longer waiting"}</span></span>{highlighted && <Badge tone="orange"><AlertTriangle size={12} /> Preference check</Badge>}{editing && <ChevronDown size={15} className="shrink-0 text-[var(--muted)]" />}</>;
    const slotElement = editing ? <button type="button" className={`focus-ring flex w-full items-center gap-2 rounded-xl border px-3 py-2 text-left ${borderClass}`} onClick={() => setPickerTarget({ team, slot })}>{content}</button> : <div className={`flex items-center gap-2 rounded-xl border px-3 py-2 ${borderClass}`}>{content}</div>;
...
function QueueView({ sessionId }: { sessionId: string }) {
  const workspaceQuery = useQuery({ queryKey: ["workspace", sessionId], queryFn: api.workspace });
  const queueQuery = useQuery({ queryKey: ["queue", sessionId], queryFn: () => api.queue(sessionId), refetchInterval: 8000 });
  return <div className="space-y-5"><QueueViewContent sessionId={sessionId} />{queueQuery.data && <SynergyTeamsCard sessionId={sessionId} queue={queueQuery.data} ended={workspaceQuery.data?.status === "ENDED"} onChanged={() => { void queueQuery.refetch(); }} />}{workspaceQuery.data && <EndSessionControl session={workspaceQuery.data} />}</div>;
}
