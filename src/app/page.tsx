...
import { RankingTrackingLinks, TrackRankingLink } from "@/components/ranking-tracking";
import { DidNotPlaySection } from "@/components/did-not-play";
import { PaymentPlayerCombobox } from "@/components/payment-player-combobox";
...
const QUEUE_SORT_LABELS: ReadonlyArray<{ key: QueueSortKey; label: string; initialDirection: QueueSortDirection }> = [
  { key: "PLAYER", label: "Player", initialDirection: "asc" },
  { key: "SKILL", label: "Skill", initialDirection: "asc" },
  { key: "GAMES", label: "Games", initialDirection: "desc" },
  { key: "WAIT", label: "Wait", initialDirection: "desc" },
  { key: "RECORD", label: "Record", initialDirection: "desc" },
];

function parseQueueSort(value: string | null): QueueSort {
  if (!value) return DEFAULT_QUEUE_SORT;
  try {
    const parsed = JSON.parse(value) as Partial<QueueSort>;
    const option = QUEUE_SORT_LABELS.find((item) => item.key === parsed.key);
    if (!option || (parsed.direction !== "asc" && parsed.direction !== "desc")) return DEFAULT_QUEUE_SORT;
    return { key: option.key, direction: parsed.direction };
  } catch {
    return DEFAULT_QUEUE_SORT;
  }
}

function nextQueueSort(current: QueueSort, key: QueueSortKey): QueueSort {
  if (current.key === key) return { key, direction: current.direction === "asc" ? "desc" : "asc" };
  return { key, direction: QUEUE_SORT_LABELS.find((item) => item.key === key)?.initialDirection ?? "asc" };
}

function queueSortDirectionLabel(sort: QueueSort) {
  return sort.direction === "asc" ? "ascending" : "descending";
}

function queueSortStorageKey(sessionId: string) {
  let accountId = "anonymous";
  if (typeof window !== "undefined") {
    try { accountId = window.localStorage.getItem("shuttle-queue-current-account") ?? accountId; } catch { /* storage may be unavailable */ }
  }
  return `bq-queue-sort:${accountId}:${sessionId}`;
}
type MatchmakingSelection = "SAME_SKILL" | "BALANCED_1" | "BALANCED_2" | "BALANCED_3" | "GUIDED" | "MIXED_DOUBLES" | "SAME_GENDER" | "OPEN" | "UNDEFEATED_CHALLENGE";
const MATCHMAKING_MODE_OPTIONS_ALL = [
  { value: "SAME_SKILL", label: "Same skill" },
  { value: "BALANCED_1", label: "Handicap +1" },
  { value: "BALANCED_2", label: "Handicap +2" },
  { value: "BALANCED_3", label: "Handicap +3" },
  { value: "GUIDED", label: "Guided" },
  { value: "MIXED_DOUBLES", label: "Mixed doubles" },
  { value: "SAME_GENDER", label: "Same gender" },
  { value: "OPEN", label: "Open" },
  { value: "UNDEFEATED_CHALLENGE", label: "Undefeated challenge" },
] as const satisfies ReadonlyArray<{ value: MatchmakingSelection; label: string }>;
const DOMAIN_SKILLS = ["NEWBIE", "BEGINNER", "UPPER_BEGINNER", "INTERMEDIATE", "UPPER_INTERMEDIATE", "ADVANCED"] as const;
const DOMAIN_GENDERS = ["MALE", "FEMALE"] as const;
const DOMAIN_QUEUE_STATUSES = ["INACTIVE", "WAITING", "QUEUED", "PLAYING", "RESTING", "CHECKED_OUT"] as const;
const isDomainSkill = (value: string): value is DomainSkillLevel => DOMAIN_SKILLS.includes(value as DomainSkillLevel);
const isDomainGender = (value: string): value is DomainGender => DOMAIN_GENDERS.includes(value as DomainGender);
const isDomainQueueStatus = (value: string): value is DomainQueuePlayerStatus => DOMAIN_QUEUE_STATUSES.includes(value as DomainQueuePlayerStatus);
...
type GuidedRequestContextValue = {
  requestId: number;
  pending: boolean;
  request: () => void;
  setPending: (pending: boolean) => void;
  availability: QueueState["guided"] | null;
};
const GuidedRequestContext = createContext<GuidedRequestContextValue>({
  requestId: 0,
  pending: false,
  request: () => undefined,
...
type QueueWorkspaceContextValue = {
...
const QueueWorkspaceContext = createContext<QueueWorkspaceContextValue>({
  queue: undefined,
  courts: [],
  workspace: undefined,
  queuePending: true,
...
function isCourtStartable(court: Pick<Court, "status" | "currentMatchId">) { return court.status === "AVAILABLE" && !court.currentMatchId; }
function matchmakingLabel(match: Pick<Match, "matchmakingMode" | "source" | "suggestionExplanation">) {
  const explanation = match.suggestionExplanation && typeof match.suggestionExplanation === "object" ? match.suggestionExplanation as { strengthGap?: unknown; challenge?: { appliedDisadvantage?: unknown; equalStrengthFallback?: unknown } } : null;
  ...
  if (match.matchmakingMode === "UNDEFEATED_CHALLENGE") {
    const disadvantage = Number(explanation?.challenge?.appliedDisadvantage);
    if (disadvantage === 1 || disadvantage === 2) return `Challenge +${disadvantage}`;
    if (explanation?.challenge?.equalStrengthFallback === true || disadvantage === 0) return "Equal-strength challenge";
    return "Undefeated challenge";
  }
  if (match.matchmakingMode === "SAME_SKILL") return "Same skill";
  if (match.matchmakingMode === "MIXED_DOUBLES") return "Mixed doubles";
  if (match.matchmakingMode === "SAME_GENDER") return "Same gender";
  if (match.matchmakingMode === "OPEN") return "Open";
  return match.source === "MANUAL_ADJUSTED" ? "Manual Adjusted" : "Manual";
}
function MatchmakingBadge({ match }: { match: Pick<Match, "matchmakingMode" | "source" | "suggestionExplanation"> }) {
  const label = matchmakingLabel(match);
  if (label === "Guided") return <Badge tone="teal">Guided</Badge>;
  if (label === "Undefeated challenge" || label.startsWith("Challenge +") || label === "Equal-strength challenge") return <Badge tone="orange">{label}</Badge>;
  if (label.startsWith("Handicap")) return <Badge tone="orange">{label}</Badge>;
...
function handicapAdvantageLabel(teamATotal: number, teamBTotal: number) {
  ...
  return `Handicap +${difference} · ${teamATotal > teamBTotal ? "Team A" : "Team B"} strength advantage`;
}

function challengeDifficultyMessage(suggestion: Suggestion) {
  const challenge = suggestion.explanation?.challenge;
  if (!challenge || challenge.appliedDisadvantage === null || challenge.appliedDisadvantage === undefined) return null;
  if (challenge.equalStrengthFallback || challenge.appliedDisadvantage === 0) return "Equal-strength challenge. No legal +1 or +2 challenge was available.";
  return `Challenge +${challenge.appliedDisadvantage}: the qualifier's opponent team is stronger by ${challenge.appliedDisadvantage}.`;
}

function challengeNoMatchGuidance(reason?: string) {
  if (reason === "NO_QUALIFIER") return "No current top-three player has reached the five-match undefeated threshold yet.";
  if (reason === "QUALIFIERS_UNAVAILABLE") return "A qualifier exists, but is resting, queued, or playing. Wait until they are waiting and rest-ready.";
  if (reason === "INSUFFICIENT_READY_PLAYERS") return "Wait for four eligible players to become waiting and rest-ready.";
  if (reason === "ALTERNATIVES_EXHAUSTED") return "All allowed Challenge alternatives were excluded. Restart Challenge suggestions to clear the cycle.";
  if (reason === "NO_ALLOWED_STRENGTH_DIFFERENCE") return "No legal +1, +2, or equal-strength lineup was found. Retry after the queue changes.";
  return "Review locked Synergy Teams and player availability, then retry after the queue changes.";
}

function rotationMessage(suggestion: Suggestion) {
  if (suggestion.mode === "UNDEFEATED_CHALLENGE") return challengeDifficultyMessage(suggestion) ?? "Challenge support players were selected under the current rotation constraints.";
...
function canConvertToManual(error: unknown) {
  if (error instanceof ApiError && typeof error.details === "object" && error.details !== null && (error.details as { canConvertToManual?: unknown }).canConvertToManual === true) return true;
  return error instanceof Error && /^(GUIDED_COMPOSITION|BALANCE_CONSTRAINT_VIOLATION|MIXED_DOUBLES_COMPOSITION|GENERATED_GENDER_RULE|MODE_CONSTRAINT_VIOLATION|UNDEFEATED_CHALLENGE_CONSTRAINT):/.test(error.message);
}
...
function DestructiveConfirmDialog({ title, body, confirmLabel, pending, error, secondaryLabel, onSecondary, onConfirm, onClose, returnFocusTarget }: { title: string; body: string; confirmLabel: string; pending: boolean; error?: unknown; secondaryLabel?: string; onSecondary?: () => void; onConfirm: () => void; onClose: () => void; returnFocusTarget?: HTMLElement | null }) {
  const dialogRef = useRef<HTMLDivElement>(null);
  const returnFocusRef = useRef<HTMLElement | null>(returnFocusTarget ?? (typeof document !== "undefined" && document.activeElement instanceof HTMLElement ? document.activeElement : null));
  useEffect(() => {
...
function GuidedAvailabilityBanner() {
  const { queue, workspace } = useContext(QueueWorkspaceContext);
  const { availability: providedAvailability, pending, request } = useContext(GuidedRequestContext);
  const availability = queue ? providedAvailability ?? guidedQueueAvailability(queue) : null;
  if (!availability || !queue) return null;
  if (!availability.available || workspace?.status === "ENDED") return null;
  const handleRequest = () => {
    request();
    const heading = document.getElementById("matchmaker-heading");
    if (heading instanceof HTMLElement) {
      const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      heading.scrollIntoView({ behavior: reducedMotion ? "auto" : "smooth", block: "center" });
      heading.focus({ preventScroll: true });
    }
  };
  return (
    <section aria-labelledby="guided-opportunity-title">
      <Card className="border-[#b8ded1] bg-[#edf8f4]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p id="guided-opportunity-title" className="font-semibold text-[var(--teal-dark)]">Guided matchup available</p>
            <p className="mt-1 text-sm text-[var(--teal-dark)]">{availability.readyLearnerCount} ready Newbie/Beginner learner{availability.readyLearnerCount === 1 ? "" : "s"} and {availability.readyGuideCount} ready Intermediate guide{availability.readyGuideCount === 1 ? "" : "s"} can form two guided teams.</p>
            <p className="mt-1 text-xs text-[var(--muted)]">Waiting totals: {availability.waitingLearnerCount} learner{availability.waitingLearnerCount === 1 ? "" : "s"} · {availability.waitingGuideCount} guide{availability.waitingGuideCount === 1 ? "" : "s"}</p>
          </div>
          <Button className="shrink-0" onClick={handleRequest} disabled={pending} loading={pending}>{pending ? "Loading Guided lineup…" : "Generate Guided lineup"}</Button>
        </div>
      </Card>
    </section>
  );
...
function ManualAdvisoryHost({ ended = false }: { ended?: boolean }) {
  ...
  return <>{!ended && request && <MatchupAdvisoryDialog advisory={request.advisory} teamA={request.teamA} teamB={request.teamB} queued={false} pending={false} onConfirm={() => { request.confirm(); setRequest(null); }} onEdit={() => setRequest(null)} onAlternative={() => setRequest(null)} onClose={() => setRequest(null)} />}</>;
}
...
function CourtCardContent({ court, match, onScore, onCancel, onEdit, onRename, onDelete, disabled = false }: { court: Court; match: Match | undefined; onScore: (match: Match) => void; onCancel: (match: Match) => void; onEdit: (match: Match) => void; onRename: (court: Court) => void; onDelete: (court: Court) => void; disabled?: boolean }) {
  ...
  return <Card data-testid="live-court-card" data-court-id={court.id} aria-labelledby={`court-${court.id}`} className={`live-court-card relative overflow-visible ${playing ? "border-[var(--teal)]" : ""}`}>
    {match && (match.matchmakingMode === "BALANCED" || match.matchmakingMode === "GUIDED" || match.matchmakingMode === "UNDEFEATED_CHALLENGE") && <div className="mb-2"><MatchmakingBadge match={match} /></div>}
...
function CourtCard(props: Parameters<typeof CourtCardContent>[0]) {
  return <CourtCardContent {...props} />;
}
...
function QueuedMatchEditDialog({ match, players, pending, error, onConfirm, onClose }: { match: Match; players: SessionPlayer[]; pending: boolean; error?: unknown; onConfirm: (teamA: string[], teamB: string[], overrideToManual?: boolean) => void; onClose: () => void }) {
...
function LiveMatchEditDialog({ match, players, courts, matches, pending, error, onConfirm, onClose }: { match: Match; players: SessionPlayer[]; courts: Court[]; matches: Match[]; pending: boolean; error?: unknown; onConfirm: (teamA: string[], teamB: string[], courtId?: string, swapWithMatchId?: string, overrideToManual?: boolean) => void; onClose: () => void }) {
  ...
  const courtOptions = courts.filter((court) => court.id === match.courtId || (court.status === "AVAILABLE" && !court.currentMatchId) || (court.status === "OCCUPIED" && court.currentMatchId && court.currentMatchId !== match.id && matches.some((item) => item.id === court.currentMatchId && item.status === "IN_PROGRESS")));
  ...
  const valid = teamA.length === teamB.length && [1, 2].includes(teamA.length) && new Set([...teamA, ...teamB]).size === teamA.length + teamB.length && hasCompleteLockedSelection(teamA, teamB, players);
  const labelFor = (id: string) => players.find((player) => player.id === id)?.displayName ?? "Select player";
  const submit = (overrideToManual = false) => {
    if (targetMatch && !swapConfirmed) { setSwapConfirmed(true); return; }
    onConfirm(teamA, teamB, courtId || undefined, targetMatch?.id, overrideToManual);
  };
  return <div className="fixed inset-0 z-50 grid items-end bg-[#102a2d]/45 p-0 sm:items-center sm:p-5">
    <div role="dialog" aria-modal="true" aria-labelledby="edit-live-match-title" className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:mx-auto sm:max-w-lg sm:rounded-3xl">
      ...
      {canConvertToManual(error) && <p className="mt-3 rounded-2xl bg-[#fffaf5] px-4 py-3 text-xs text-[#8d4824]">This lineup no longer satisfies its original matchmaking guarantee. Save it as Manual Adjusted to keep player, Synergy Team, rest, and court safety checks.</p>}
      <div className="mt-5 grid gap-2 sm:grid-cols-[1fr_1fr_1.2fr]"><Button variant="quiet" className="w-full" onClick={swapTeams} disabled={pending}>Swap teams</Button><Button variant="quiet" className="w-full" onClick={onClose} disabled={pending}>Cancel</Button><Button className="w-full" onClick={() => submit()} disabled={!valid || pending} loading={pending}>{targetMatch && !swapConfirmed ? "Review court swap" : targetMatch ? "Confirm court swap" : "Save live match"}</Button>{canConvertToManual(error) && <Button variant="quiet" className="w-full" onClick={() => submit(true)} disabled={!valid || pending}>Save as Manual Adjusted</Button>}</div>
    </div>
  </div>;
}

function QueuedMatchCardContent({ match, courts, onChanged, onCancel, onEdit, disabled = false }: { match: Match; courts: Court[]; onChanged: () => void; onCancel: (match: Match) => void; onEdit: (match: Match) => void; disabled?: boolean }) {
  ...
  const availableCourts = courts.filter(isCourtStartable);
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (!courtId || courts.some((court) => court.id === courtId && isCourtStartable(court))) return;
    setCourtId("");
    toast.message("The selected court is no longer available. Choose another court before starting.");
  }, [courtId, courts]);
...
function LiveView({ session, sessionId, onStartSession }: { session: SessionSummary; sessionId: string; onStartSession: () => void }) {
  ...
  return <div className="live-view space-y-5"><span className="sr-only">Queued matchups</span>
    ...
    {editMatch?.status === "IN_PROGRESS" && <LiveMatchEditDialog match={editMatch} players={editablePlayers} courts={courts} matches={matches} pending={edit.isPending} error={edit.isError ? edit.error : undefined} onConfirm={(teamA, teamB, courtId, swapWithMatchId, overrideToManual) => edit.mutate({ teamA, teamB, ...(courtId ? { courtId } : {}), ...(swapWithMatchId ? { swapWithMatchId } : {}), ...(overrideToManual ? { overrideToManual: true } : {}) })} onClose={() => { if (!edit.isPending) { edit.reset(); setEditMatch(null); } }} />}
    {editMatch?.status === "QUEUED" && <QueuedMatchEditDialog match={editMatch} players={editablePlayers} pending={edit.isPending} error={edit.isError ? edit.error : undefined} onConfirm={(teamA, teamB, overrideToManual) => edit.mutate({ teamA, teamB, ...(overrideToManual ? { overrideToManual: true } : {}) })} onClose={() => { if (!edit.isPending) { edit.reset(); setEditMatch(null); } }} />}
...
function waitMinutes(player: SessionPlayer, serverTime?: string, minimumRestMinutes = 0) {
  const minutes = queueWaitMinutes(player, serverTime ?? new Date().toISOString(), minimumRestMinutes);
...
function WaitIndicator({ player, serverTime, minimumRestMinutes }: { player: SessionPlayer; serverTime: string; minimumRestMinutes: number }) {
  const minutes = queueWaitMinutes(player, serverTime, minimumRestMinutes);
  const attention = waitAttention(minutes);
  const mobileDetails = <span className="mt-1 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-[var(--muted)] md:hidden" aria-label={`${pretty(player.skillLevel)} skill, ${player.matchesPlayed} games, ${player.wins} wins and ${player.losses} losses`}><span><span className="font-semibold text-[var(--ink)]">Skill</span> {pretty(player.skillLevel)}</span><span><span className="font-semibold text-[var(--ink)]">Games</span> {player.matchesPlayed}</span><span><span className="font-semibold text-[var(--ink)]">Wait</span> {minutes === null ? "—" : `${minutes}m`}</span><span><span className="font-semibold text-[var(--ink)]">Record</span> {player.wins}W / {player.losses}L</span></span>;
  if (!attention) return mobileDetails;
  const label = attention === "VERY_LONG" ? `Very long wait · ${minutes}m` : `Long wait · ${minutes}m`;
  return <>{mobileDetails}<span title="Wait counts time available to play after required rest." className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-semibold ${attention === "VERY_LONG" ? "bg-[#fff0e4] text-[#8d4824]" : "bg-[#fff8dc] text-[#7a5b00]"}`}><Clock3 size={12} aria-hidden="true" />{label}</span></>;
}
...
function PlayerQueueRow({ player, sessionId, serverTime, minimumRestMinutes, onChanged, onRemove, canRemove = true, removePending = false, disabled = false, selected = false, onToggle, bulkPending = false }: { player: SessionPlayer; sessionId: string; serverTime: string; minimumRestMinutes: number; onChanged: () => void; onRemove: (player: SessionPlayer) => void; canRemove?: boolean; removePending?: boolean; disabled?: boolean; selected?: boolean; onToggle?: (checked: boolean) => void; bulkPending?: boolean }) {
...
function normalizeMatchmakingSelection(mode: string): MatchmakingSelection { return mode === "BALANCED" ? "BALANCED_1" : (MATCHMAKING_MODE_OPTIONS_ALL.some((option) => option.value === mode) ? mode : "SAME_SKILL") as MatchmakingSelection; }
...
function matchModeLabel(mode: string) { return mode === "BALANCED_3" ? "Handicap +3 strength" : mode === "BALANCED_2" ? "Handicap +2 strength" : mode === "BALANCED_1" || mode === "BALANCED" ? "Handicap +1 strength" : mode === "SAME_SKILL" ? "Same skill" : mode === "GUIDED" ? "Guided" : mode; }

function toDomainGuidedPlayer(player: SessionPlayer): DomainMatchPlayer | null {
  if (!isDomainSkill(player.skillLevel) || !isDomainGender(player.gender) || !isDomainQueueStatus(player.status)) return null;
  const effectiveSkillLevel = player.effectiveSkillLevel;
  const parsedEffectiveSkillLevel = effectiveSkillLevel && isDomainSkill(effectiveSkillLevel) ? effectiveSkillLevel : undefined;
  return {
    id: player.id,
    ...
    ...(typeof player.effectiveSkillWeight === "number" ? { effectiveSkillWeight: player.effectiveSkillWeight } : {}),
    ...(parsedEffectiveSkillLevel ? { effectiveSkillLevel: parsedEffectiveSkillLevel } : {}),
    synergyTeamId: player.synergyTeamId ?? null,
    status: player.status,
    gamesPlayed: player.matchesPlayed,
    wins: player.wins,
    losses: player.losses,
    queueEnteredAt: player.queueEnteredAt ?? null,
    lastMatchEndedAt: player.lastMatchEndedAt ?? null,
    manualPriority: player.manualPriority ?? 0,
    latePenaltyState: player.latePenaltyState,
  };
}

function toDomainMatchmakingPlayer(player: SessionPlayer): DomainMatchPlayer | null {
  const guided = toDomainGuidedPlayer(player);
  ...
  return {
    ...
    lastMatchEndedAt: player.lastMatchEndedAt ?? null,
    manualPriority: player.manualPriority ?? 0,
    latePenaltyState: player.latePenaltyState ?? null,
  };
}

function guidedQueueAvailability(queue: QueueState) {
  const guided = queue.guided;
  const validReason = guided?.reason === null || guided?.reason === "NO_GUIDED_COMPOSITION" || guided?.reason === "REST_REQUIRED" || guided?.reason === "NO_VALID_GROUP";
  const validCounts = guided ? [guided.waitingLearnerCount, guided.waitingGuideCount, guided.readyLearnerCount, guided.readyGuideCount].every((count) => Number.isInteger(count) && count >= 0) : false;
  const validNextEligibleAt = guided?.nextEligibleAt === null || typeof guided?.nextEligibleAt === "string";
  if (guided && typeof guided.available === "boolean" && validReason && validCounts && validNextEligibleAt) return guided;
  const guidedPlayers = [...queue.waiting, ...queue.resting, ...queue.queued, ...queue.playing, ...queue.inactive].map(toDomainGuidedPlayer).filter((player): player is DomainMatchPlayer => Boolean(player));
  return evaluateGuidedAvailability(guidedPlayers, { minimumRestMinutes: queue.minimumRestMinutes ?? 0, now: queue.serverTime, synergyTeams: queue.synergyTeams ?? [] });
}

function deriveChallengePlayers(queue: QueueState) {
  const allPlayers = [...queue.waiting, ...queue.resting, ...queue.queued, ...queue.playing, ...queue.inactive].map(toDomainGuidedPlayer).filter((player): player is DomainMatchPlayer => Boolean(player));
...
function MatchmakerPanel({ sessionId, queue, courts, onChanged, queueLoaded = true, guidedRequestId: guidedRequestIdProp = 0, onGuidedPendingChange: onGuidedPendingChangeProp }: { sessionId: string; queue: QueueState; courts: Court[]; onChanged: () => void; queueLoaded?: boolean; guidedRequestId?: number; onGuidedPendingChange?: (pending: boolean) => void }) {
  const guidedRequestContext = useContext(GuidedRequestContext);
  const guidedRequestId = guidedRequestIdProp || guidedRequestContext.requestId;
  const onGuidedPendingChange = onGuidedPendingChangeProp ?? guidedRequestContext.setPending;
  ...
  const [mode, setMode] = useState<MatchmakingSelection>(() => normalizeMatchmakingSelection(persisted.mode));
  ...
  const [noMatch, setNoMatch] = useState<{ code: SuggestionNoMatchCode; message: string; nextEligibleAt?: string | null; readyMaleCount?: number; readyFemaleCount?: number; waitingMaleCount?: number; waitingFemaleCount?: number; readyLearnerCount?: number; readyGuideCount?: number; waitingLearnerCount?: number; waitingGuideCount?: number; challenge?: { qualifierCount?: number; readyQualifierCount?: number; readyPlayerCount?: number; reason?: "NO_QUALIFIER" | "QUALIFIERS_UNAVAILABLE" | "INSUFFICIENT_READY_PLAYERS" | "INCOMPATIBLE_LINEUP_CONSTRAINTS" | "NO_ALLOWED_STRENGTH_DIFFERENCE" | "ALTERNATIVES_EXHAUSTED" } } | null>(null);
  const [serverConstraintError, setServerConstraintError] = useState<string | null>(null);
  const [guidedRequestPending, setGuidedRequestPending] = useState(false);
  const [overrideToManual, setOverrideToManual] = useState(false);
  const matchmakerHeadingRef = useRef<HTMLHeadingElement>(null);
  const suggestionResultRef = useRef<HTMLDivElement>(null);
  const noMatchResultRef = useRef<HTMLDivElement>(null);
  const bannerGuidedRequestRef = useRef(false);
  ...
  const challengeStatus = queue.undefeatedChallenge;
  const challengePlayers = useMemo(() => challengeStatus && challengeStatus.minimumMatches === UNDEFEATED_CHALLENGE_MINIMUM_MATCHES && challengeStatus.rankLimit === UNDEFEATED_CHALLENGE_RANK_LIMIT ? challengeStatus.players : deriveChallengePlayers(queue), [challengeStatus, queue]);
  const readyChallengePlayers = challengePlayers.filter((player) => player.ready);
  const MATCHMAKING_MODE_OPTIONS = useMemo(() => challengePlayers.length > 0 ? MATCHMAKING_MODE_OPTIONS_ALL : MATCHMAKING_MODE_OPTIONS_ALL.filter((option) => option.value !== "UNDEFEATED_CHALLENGE"), [challengePlayers.length]);
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (mode === "UNDEFEATED_CHALLENGE" && challengePlayers.length === 0) {
      setMode("SAME_SKILL");
      setSuggestion(null);
      setDraft(null);
      setSuggestionScope("");
      ...
      setOverrideToManual(false);
      setNoMatch({ code: "NO_UNDEFEATED_QUALIFIER", message: "The player no longer qualifies for Undefeated Challenge. Same skill is selected." });
    }
  }, [challengePlayers.length, mode]);
  /* eslint-enable react-hooks/set-state-in-effect */
  ...
  useEffect(() => {
    if (guidedAvailability.available && previousGuidedAvailability.current !== true) toast.success("A Guided matchup is ready.");
    previousGuidedAvailability.current = guidedAvailability.available;
  }, [guidedAvailability.available]);
  useEffect(() => {
    if (mode !== "UNDEFEATED_CHALLENGE" || !noMatch) return;
    const frame = window.requestAnimationFrame(() => {
      const status = [...document.querySelectorAll<HTMLElement>('[role="status"][tabindex="-1"]')].at(-1);
      (status ?? matchmakerHeadingRef.current)?.focus();
    });
    return () => window.cancelAnimationFrame(frame);
  }, [mode, noMatch]);
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (mode !== "UNDEFEATED_CHALLENGE" || !noMatch?.challenge || noMatch.message.includes("Qualifiers:")) return;
    const challenge = noMatch.challenge;
    const detail = ` ${challengeNoMatchGuidance(challenge.reason)} Qualifiers: ${challenge.qualifierCount ?? 0}; ready qualifiers: ${challenge.readyQualifierCount ?? 0}; ready players: ${challenge.readyPlayerCount ?? 0}.`;
    setNoMatch((current) => current && !current.message.includes("Qualifiers:") ? { ...current, message: `${current.message}${detail}` } : current);
  }, [mode, noMatch]);
  /* eslint-enable react-hooks/set-state-in-effect */
  const notifiedChallengeIds = useRef(new Set<string>());
  const challengeTrackerInitialized = useRef(false);
  useEffect(() => {
    if (!queueLoaded) return;
    const currentIds = new Set(challengePlayers.map((player) => player.queuePlayerId));
    if (!challengeTrackerInitialized.current) {
      currentIds.forEach((id) => notifiedChallengeIds.current.add(id));
      challengeTrackerInitialized.current = true;
      return;
    }
    currentIds.forEach((id) => {
      if (notifiedChallengeIds.current.has(id)) return;
      const player = challengePlayers.find((candidate) => candidate.queuePlayerId === id);
      if (player) toast.success(`${player.displayName} unlocked Undefeated Challenge with a ${player.wins}–0 record.`);
      notifiedChallengeIds.current.add(id);
    });
    [...notifiedChallengeIds.current].forEach((id) => { if (!currentIds.has(id)) notifiedChallengeIds.current.delete(id); });
  }, [challengePlayers, queueLoaded]);
  useEffect(() => {
    const listener = (event: Event) => {
      const notifications = (event as CustomEvent<Array<{ queuePlayerId: string; displayName: string; wins: number }>>).detail ?? [];
      for (const notification of notifications) {
        if (notifiedChallengeIds.current.has(notification.queuePlayerId)) continue;
        notifiedChallengeIds.current.add(notification.queuePlayerId);
        toast.success(`${notification.displayName} unlocked Undefeated Challenge with a ${notification.wins}–0 record.`);
      }
    };
    window.addEventListener("undefeated-challenge-eligible", listener);
    return () => window.removeEventListener("undefeated-challenge-eligible", listener);
  }, []);
  ...
  const lineupEdited = Boolean(suggestion && draft && (suggestion.teamA.some((player, index) => draft.teamA[index] !== player.id) || suggestion.teamB.some((player, index) => draft.teamB[index] !== player.id)));
  const draftDomainTeamA = draftTeamAPlayers.map(toDomainMatchmakingPlayer).filter((player): player is DomainMatchPlayer => Boolean(player));
  const draftDomainTeamB = draftTeamBPlayers.map(toDomainMatchmakingPlayer).filter((player): player is DomainMatchPlayer => Boolean(player));
  const draftModeResult = mode === "UNDEFEATED_CHALLENGE" && !lineupEdited
    ? null
    : mode === "UNDEFEATED_CHALLENGE"
      ? { valid: false as const, message: "Edited Undefeated Challenge lineups must continue as Manual Adjusted." }
      : draftDomainTeamA.length === draftTeamAPlayers.length && draftDomainTeamB.length === draftTeamBPlayers.length && draftTeamAPlayers.length === draftTeamBPlayers.length && draftTeamAPlayers.length > 0
        ? validateMatchmakingConstraints(suggestionApiMode(mode) as DomainMatchmakingMode, draftDomainTeamA, draftDomainTeamB, strengthGap ?? 1)
        : null;
  const draftConstraintError = draftModeResult && !draftModeResult.valid ? draftModeResult.message : serverConstraintError;
  ...
  const isAdjusted = lineupEdited;
  const challengeEditBlocked = mode === "UNDEFEATED_CHALLENGE" && isAdjusted && !overrideToManual;
  const draftReady = Boolean(draft && draft.teamA.length === 2 && draft.teamB.length === 2 && new Set(draftIds).size === 4 && unavailableIds.length === 0 && !challengeEditBlocked && (!draftConstraintError || (overrideToManual && isAdjusted)));
  const expired = Boolean(suggestion && suggestion.expiresAt <= clock);
  const availableCourts = courts.filter(isCourtStartable);
  const selectedCourtStale = Boolean(courtId && !availableCourts.some((court) => court.id === courtId));
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (!selectedCourtStale) return;
    setCourtIdRaw("");
    toast.message("The selected court is no longer available. Choose another court.");
  }, [selectedCourtStale]);
  /* eslint-enable react-hooks/set-state-in-effect */
  ...
  type SuggestMutationVariables = { requestedMode?: MatchmakingSelection; requestedExcludeKeys?: string[]; banner?: boolean };
  const suggest = useMutation({ mutationFn: ({ requestedMode = mode, requestedExcludeKeys = excludedSuggestionKeys }: SuggestMutationVariables = {}) => api.suggestions(sessionId, suggestionApiMode(requestedMode), suggestionStrengthGap(requestedMode), requestedExcludeKeys), onSuccess: (result, variables) => { const selectedMode = variables?.requestedMode ?? mode; const selectedGap = suggestionStrengthGap(selectedMode) ?? persisted.strengthGap; const selectedScope = `${selectedMode}|${selectedGap}|${waitingFingerprint}`; if (!result.suggestion) { setNoMatch(result.noMatch ?? null); if (!variables?.banner && !bannerGuidedRequestRef.current) toast.message(result.noMatch?.message ?? "No valid group is available yet."); bannerGuidedRequestRef.current = false; return; } setNoMatch(null); const nextSuggestion = result.suggestion; setSuggestion(nextSuggestion); setDraft({ teamA: nextSuggestion.teamA.map((player) => player.id), teamB: nextSuggestion.teamB.map((player) => player.id) }); setSuggestionScope(selectedScope); setMode(selectedMode); setEditing(false); setPickerTarget(null); setCourtIdRaw(availableCourts[0]?.id ?? ""); setSuggestionCycle((current) => { const keys = result.cycleRestarted || current.scope !== selectedScope ? [] : current.keys; return { scope: selectedScope, keys: [...new Set([...keys, nextSuggestion.key])].slice(-50) }; }); bannerGuidedRequestRef.current = false; }, onError: (error) => { bannerGuidedRequestRef.current = false; toast.error(errorMessage(error)); } });
  const createSuggested = useMutation({ mutationFn: ({ startNow, playerPreferenceConfirmed = false }: { startNow: boolean; playerPreferenceConfirmed?: boolean }) => { if (!suggestion || !draft || !draftReady) throw new Error(draftConstraintError ?? "Every selected player must still be waiting."); if (expired) throw new Error("This suggestion expired. Generate a new suggestion or continue in Manual mode."); if (startNow && !courtId) throw new Error("Select an available court first."); return api.createSuggestedMatch(sessionId, { teamA: draft.teamA, teamB: draft.teamB, suggestionToken: suggestion.token, suggestionAdjusted: isAdjusted, ...(overrideToManual ? { overrideToManual: true } : {}), ...(playerPreferenceConfirmed ? { playerPreferenceConfirmed: true } : {}), ...(startNow ? { courtId } : {}) }); }, onSuccess: (_match, variables) => { setAdvisoryOpen(false); setSuggestion(null); setDraft(null); setSuggestionScope(""); setSuggestionCycle({ scope: "", keys: [] }); setEditing(false); setPickerTarget(null); setCourtId(""); setOverrideToManual(false); onChanged(); toast.success(variables.startNow ? "Match started." : "Match queued."); }, onError: (error) => toast.error(errorMessage(error)) });
  /* A server-side mode rejection can happen after an edit that the client
   * cannot fully validate (for example a stale player snapshot). Surface the
   * same conversion action as local validation when the API says it is safe. */
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (createSuggested.isError && isAdjusted && canConvertToManual(createSuggested.error)) {
      setServerConstraintError(errorMessage(createSuggested.error));
    } else if (!createSuggested.isError && serverConstraintError) {
      setServerConstraintError(null);
    }
  }, [createSuggested.error, createSuggested.isError, isAdjusted, serverConstraintError]);
  /* eslint-enable react-hooks/set-state-in-effect */
  ...
  const changeMode = (nextMode: string) => { const normalized = normalizeMatchmakingSelection(nextMode); if (normalized === mode) return; if (isAdjusted && !window.confirm("Discard your edited lineup and generate in the new mode?")) return; setMode(normalized); setOverrideToManual(false); setNoMatch(null); setSuggestion(null); setDraft(null); setSuggestionScope(""); setSuggestionCycle({ scope: "", keys: [] }); setEditing(false); setPickerTarget(null); setCourtId(""); };
  const requestSuggestion = () => {
    const fallbackMode = noMatch?.code === "NO_EXACT_STRENGTH_GAP" ? nextTighterMatchMode(mode) : null;
    if (fallbackMode && window.confirm(`No exact ${matchModeLabel(mode)} lineup is available. Try ${matchModeLabel(fallbackMode)} instead?`)) {
      changeMode(fallbackMode);
      return;
    }
    if (isAdjusted && !window.confirm("Discard this edited lineup and generate another suggestion?")) return;
    setOverrideToManual(false);
    suggest.mutate({});
  };
  const requestChallengeSuggestion = () => { if (isAdjusted && !window.confirm("Discard this edited lineup and create an Undefeated Challenge suggestion?")) return; setPanelMode("suggested"); setMode("UNDEFEATED_CHALLENGE"); setOverrideToManual(false); setNoMatch(null); setSuggestion(null); setDraft(null); setSuggestionScope(""); setSuggestionCycle({ scope: "", keys: [] }); setEditing(false); setPickerTarget(null); setCourtIdRaw(""); let requestFailed = false; suggest.mutate({ requestedMode: "UNDEFEATED_CHALLENGE", requestedExcludeKeys: [], banner: true }, { onError: () => { requestFailed = true; }, onSettled: () => { window.requestAnimationFrame(() => { const target = requestFailed ? matchmakerHeadingRef.current : noMatchResultRef.current ?? suggestionResultRef.current ?? matchmakerHeadingRef.current; target?.focus(); }); } }); };
  const lastGuidedRequestId = useRef(0);
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (!guidedRequestId || guidedRequestId === lastGuidedRequestId.current) return;
    lastGuidedRequestId.current = guidedRequestId;
    if (isAdjusted && !window.confirm("Discard your edited lineup and generate a Guided suggestion?")) {
      onGuidedPendingChange?.(false);
      return;
    }
    setPanelMode("suggested");
    setMode("GUIDED");
    setNoMatch(null);
    setSuggestion(null);
    setDraft(null);
    ...
    setCourtIdRaw("");
    setOverrideToManual(false);
    setGuidedRequestPending(true);
  }, [guidedRequestId, isAdjusted, onGuidedPendingChange]);
  /* eslint-enable react-hooks/set-state-in-effect */
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => {
    if (!guidedRequestPending || mode !== "GUIDED") return;
    setGuidedRequestPending(false);
    bannerGuidedRequestRef.current = true;
    onGuidedPendingChange?.(true);
    let requestFailed = false;
    suggest.mutate({ requestedMode: "GUIDED", banner: true }, { onError: () => { requestFailed = true; }, onSettled: () => {
      onGuidedPendingChange?.(false);
      window.requestAnimationFrame(() => {
        const target = requestFailed ? matchmakerHeadingRef.current : noMatchResultRef.current ?? suggestionResultRef.current ?? matchmakerHeadingRef.current;
        target?.focus();
      });
    } });
  }, [guidedRequestPending, mode, onGuidedPendingChange, suggest]);
  /* eslint-enable react-hooks/set-state-in-effect */
  ...
  const transferToManual = () => { if (!draft || unavailableIds.length > 0) { toast.error("Replace unavailable players before continuing as Manual Adjusted."); return; } if (!isAdjusted) { toast.message("Edit the suggested lineup before converting it to Manual Adjusted."); return; } if (!window.confirm("Continue as Manual Adjusted? The selected mode guarantees will be removed, while player, Synergy Team, rest, and court safety rules remain enforced.")) return; setOverrideToManual(true); setEditing(false); setPickerTarget(null); };
...
function QueueViewContent({ sessionId }: { sessionId: string }) {
  const { queue, courts, workspace, queuePending, courtsPending, workspacePending, queueError } = useContext(QueueWorkspaceContext);
  const queryClient = useQueryClient();
  ...
  const ended = workspace?.status === "ENDED";
  // Keep the existing render logic simple while sourcing all queue data from
  // the single observer owned by QueueView.
  const queueQuery = { data: queue, isPending: queuePending, error: queueError };
  const courtsQuery = { data: courts, isPending: courtsPending };
  ...
  const filtered = useMemo(() => { const needle = search.trim().toLowerCase(); const matching = allPlayers.filter((player) => (filter === "ALL" || (filter === "INACTIVE" ? ["INACTIVE", "CHECKED_OUT"].includes(player.status) : player.status === filter)) && (!needle || player.displayName.toLowerCase().includes(needle))); return sortQueuePlayers(matching, sort, queue?.serverTime ?? new Date().toISOString(), queue?.minimumRestMinutes ?? 0); }, [allPlayers, filter, queue?.minimumRestMinutes, queue?.serverTime, search, sort]);
  ...
  const changeSort = (key: QueueSortKey) => { setPage(1); setSort((current) => nextQueueSort(current, key)); };
  const changeSortDirection = () => { setPage(1); setSort((current) => ({ ...current, direction: current.direction === "asc" ? "desc" : "asc" })); };
  const sortStorageKey = queueSortStorageKey(sessionId);
  /* eslint-disable react-hooks/set-state-in-effect */
  useEffect(() => { try { const stored = window.sessionStorage.getItem(sortStorageKey); if (stored) setSort(parseQueueSort(stored)); } catch { /* storage may be unavailable */ } }, [sortStorageKey]);
  useEffect(() => { try { window.sessionStorage.setItem(sortStorageKey, JSON.stringify(sort)); } catch { /* storage may be unavailable */ } }, [sort, sortStorageKey]);
  useEffect(() => { setPage((current) => Math.min(current, pageCount)); }, [pageCount]);
  /* eslint-enable react-hooks/set-state-in-effect */
  if (queuePending || courtsPending || workspacePending) return <LoadingState label="Loading queue" />;
  return <div className="space-y-5"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-[var(--muted)]">Queue operations</p><h2 className="display text-4xl">Build the next matchup.</h2></div><div className="flex items-center gap-2 text-sm text-[var(--muted)]"><UsersRound size={17} className="text-[var(--teal)]" /> {allPlayers.length} rostered players</div></div><GuidedAvailabilityBanner />{ended && <Card className="border-[#f6c49f] bg-[#fffaf5]"><p className="font-semibold">Session ended</p><p className="mt-1 text-sm text-[var(--muted)]">Check-in, matchmaking, and queue actions are locked. Review finalized fees in the Fees tab.</p></Card>}<div className="flex gap-2 overflow-x-auto pb-1">{labels.map(({ key, label }) => <button key={key} className={`focus-ring shrink-0 rounded-full border px-3 py-2 text-xs font-semibold transition ${filter === key ? "border-[var(--teal)] bg-[var(--teal)] text-white" : "border-[var(--line)] bg-white text-[var(--muted)] hover:border-[var(--teal)]"}`} onClick={() => changeFilter(key)}>{label} <span className="ml-1 opacity-75">{counts[key as Exclude<QueueFilter, "ALL">]}</span></button>)}</div>{queueQuery.error && <p role="alert" className="rounded-2xl bg-[#fff0e4] px-4 py-3 text-sm text-[#8d4824]">{errorMessage(queueQuery.error)}</p>}<div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]"><div className="min-w-0"><Card className="p-0"><div className="flex flex-col gap-3 border-b border-[var(--line)] p-4 sm:flex-row"><label className="relative block flex-1"><Search size={16} className="absolute left-3 top-3 text-[var(--muted)]" /><Input className="pl-9" value={search} onChange={(event) => changeSearch(event.target.value)} placeholder="Search players" aria-label="Search players" /></label><div className="flex items-center gap-2 text-xs text-[var(--muted)]"><Filter size={15} /> {filtered.length} shown</div></div>{selectedIds.length > 0 || selectableVisible.length > 0 ? <div className="flex flex-col gap-3 border-b border-[var(--line)] bg-[#fbfdfb] p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{selectedIds.length} selected</span><Button variant="quiet" className="px-3 py-1.5 text-xs" aria-pressed={allVisibleSelected} onClick={toggleVisibleSelection} disabled={ended || bulkAction.isPending || !selectableVisible.length}>{allVisibleSelected ? "Unselect all visible" : "Select all visible"}</Button><Button variant="quiet" className="px-3 py-1.5 text-xs" onClick={() => setSelectedIds([])} disabled={!selectedIds.length || bulkAction.isPending}>Clear</Button></div><div className="flex flex-wrap gap-2"><Button variant="quiet" className="px-3 py-1.5 text-xs" onClick={() => bulkAction.mutate("CHECK_IN")} disabled={ended || bulkAction.isPending || !actionEnabled("CHECK_IN")}>Check in</Button><Button variant="quiet" className="px-3 py-1.5 text-xs" onClick={() => bulkAction.mutate("REST")} disabled={ended || bulkAction.isPending || !actionEnabled("REST")}>Rest</Button><Button variant="quiet" className="px-3 py-1.5 text-xs" onClick={() => bulkAction.mutate("CHECK_OUT")} disabled={ended || bulkAction.isPending || !actionEnabled("CHECK_OUT")}>Check out</Button></div></div> : null}<div className="mb-3 flex flex-col gap-2 border-b border-[var(--line)] bg-[#fbfdfb] p-3 md:hidden"><label className="block text-xs font-semibold text-[var(--muted)]" htmlFor="queue-sort">Sort players<Select id="queue-sort" value={sort.key} onChange={(event) => changeSort(event.target.value as QueueSortKey)}>{QUEUE_SORT_LABELS.map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</Select></label><Button variant="quiet" className="min-h-11 w-full justify-between px-3 text-xs" onClick={changeSortDirection} aria-label={`Sort ${QUEUE_SORT_LABELS.find((option) => option.key === sort.key)?.label ?? "players"} ${sort.direction === "asc" ? "descending" : "ascending"}`}><span>{QUEUE_SORT_LABELS.find((option) => option.key === sort.key)?.label ?? "Players"} · {queueSortDirectionLabel(sort)}</span><span aria-hidden="true">{sort.direction === "asc" ? "↑" : "↓"}</span></Button></div><div role="row" className={`hidden ${QUEUE_TABLE_GRID_COLUMNS} items-center gap-2 border-b border-[var(--line)] bg-[#fbfdfb] px-3 py-3 text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted)] md:grid`}>{QUEUE_SORT_LABELS.map((option) => <span key={option.key} role="columnheader" {...(sort.key === option.key ? { "aria-sort": sort.direction === "asc" ? "ascending" : "descending" } : {})}><button type="button" className="focus-ring flex min-h-11 w-full items-center justify-between gap-1 rounded px-1 text-left" onClick={() => changeSort(option.key)} aria-label={`Sort by ${option.label}, currently ${sort.key === option.key ? queueSortDirectionLabel(sort) : "unsorted"}`}>{option.label}<span aria-hidden="true" className="text-sm">{sort.key === option.key ? sort.direction === "asc" ? "↑" : "↓" : "↕"}</span></button></span>)}<span role="columnheader">Status</span><span role="columnheader">Action</span></div>{queueQuery.isPending ? <div className="space-y-2 p-4"><div className="h-12 animate-pulse rounded-2xl bg-[#f1f5f2]" /><div className="h-12 animate-pulse rounded-2xl bg-[#f1f5f2]" /></div> : visible.length === 0 ? <div className="p-4"><EmptyState icon={UsersRound} title="No players here" body={search ? "Try a different search or clear the filter." : "Players will appear as they check in."} /></div> : <div>{visible.map((player) => <PlayerQueueRow key={player.id} player={player} sessionId={sessionId} serverTime={queue?.serverTime ?? new Date().toISOString()} minimumRestMinutes={queue?.minimumRestMinutes ?? 0} canRemove={filter === "INACTIVE"} removePending={remove.isPending} onRemove={requestRemove} selected={selectedIds.includes(player.id)} onToggle={(checked) => toggleSelection(player.id, checked)} bulkPending={bulkAction.isPending} disabled={ended} onChanged={invalidate} />)}</div>}<div className="border-t border-[var(--line)] px-4 py-3"><p className="mb-2 text-xs text-[var(--muted)]">Wait counts time available to play after required rest. Long wait starts at {LONG_WAIT_MINUTES} minutes; very long wait starts at {VERY_LONG_WAIT_MINUTES}.</p><div className="flex items-center justify-between"><p className="text-xs text-[var(--muted)]">Page {Math.min(page, pageCount)} of {pageCount} · 15 per page</p><div className="flex gap-2"><Button variant="quiet" className="px-3" aria-label="Previous page" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}><ArrowLeft size={15} /></Button><Button variant="quiet" className="px-3" aria-label="Next page" disabled={page >= pageCount} onClick={() => setPage((current) => Math.min(pageCount, current + 1))}><ArrowRight size={15} /></Button></div></div></div>{removeCandidate && <DestructiveConfirmDialog title={`Remove ${removeCandidate.displayName} from queue?`} body="This removes the player from the current queue only. Their reusable directory profile will remain available in Players." confirmLabel="Remove" pending={remove.isPending} error={remove.isError ? remove.error : undefined} onConfirm={() => remove.mutate(removeCandidate.id)} onClose={() => { if (!remove.isPending) { remove.reset(); setRemoveCandidate(null); } }} />}</Card></div>{ended ? <Card><p className="font-semibold">Matchmaking is unavailable</p><p className="mt-1 text-sm text-[var(--muted)]">Start a fresh queue from Settings to resume operations.</p></Card> : <MatchmakerPanel key={sessionId} sessionId={sessionId} queue={queue ?? { waiting: [], resting: [], inactive: [], queued: [], playing: [], serverTime: new Date().toISOString() }} courts={courtsQuery.data ?? []} queueLoaded={Boolean(queue)} onChanged={invalidate} />}</div></div>;
}
...
function QueueView({ sessionId }: { sessionId: string }) {
  ...
  const [guidedRequestId, setGuidedRequestId] = useState(0);
  const [guidedPending, setGuidedPending] = useState(false);
  const requestGuided = useCallback(() => {
    setGuidedPending(true);
    setGuidedRequestId((current) => current + 1);
  }, []);
  const guidedAvailability = queueQuery.data ? guidedQueueAvailability(queueQuery.data) : null;
  const guidedRequestContext = useMemo<GuidedRequestContextValue>(() => ({ requestId: guidedRequestId, pending: guidedPending, request: requestGuided, setPending: setGuidedPending, availability: guidedAvailability }), [guidedAvailability, guidedPending, guidedRequestId, requestGuided]);
  const queueWorkspaceContext = useMemo<QueueWorkspaceContextValue>(() => ({ queue: queueQuery.data, courts: courtsQuery.data ?? [], workspace: workspaceQuery.data, queuePending: queueQuery.isPending, courtsPending: courtsQuery.isPending, workspacePending: workspaceQuery.isPending, queueError: queueQuery.error }), [courtsQuery.data, courtsQuery.isPending, queueQuery.data, queueQuery.error, queueQuery.isPending, workspaceQuery.data, workspaceQuery.isPending]);
  return <GuidedRequestContext.Provider value={guidedRequestContext}><QueueWorkspaceContext.Provider value={queueWorkspaceContext}><div className="space-y-5"><QueueViewContent sessionId={sessionId} />{queueQuery.data && <SynergyTeamsCard sessionId={sessionId} queue={queueQuery.data} ended={workspaceQuery.data?.status === "ENDED"} onChanged={() => { void queueQuery.refetch(); }} />}{workspaceQuery.data && <EndSessionControl session={workspaceQuery.data} />}<ManualAdvisoryHost ended={workspaceQuery.data?.status !== "ACTIVE"} /></div></QueueWorkspaceContext.Provider></GuidedRequestContext.Provider>;
}
...
function historyScoreText(match: HistoryMatch) {
...
function historyProvenance(match: HistoryMatch) {
  if (match.provenance) return match.provenance;
  if (match.source === "MANUAL") return { kind: "MANUAL" as const, label: "Manual", description: "Teams were selected directly by the Queue Master.", originalMode: null, guaranteesRetained: null };
  if (match.source === "MANUAL_ADJUSTED") return { kind: "LEGACY_ADJUSTED" as const, label: "Adjusted lineup", description: "The lineup was recorded as adjusted, but its original suggestion details are unavailable.", originalMode: null, guaranteesRetained: null };
  return { kind: "GENERATED" as const, label: match.matchmakingLabel ?? "Generated suggestion", description: `${match.matchmakingLabel ?? "Generated suggestion"} was accepted without manual lineup changes.`, originalMode: null, guaranteesRetained: true };
}

function historyScoreLabel(match: HistoryMatch) {
  const score = historyScoreText(match);
  const provenance = historyProvenance(match);
  if (provenance.kind === "MANUAL") return `${provenance.label} · ${score}`;
  if (provenance.kind === "ADJUSTED_SUGGESTION" || provenance.kind === "LEGACY_ADJUSTED") return `${provenance.label} · ${score} · ${provenance.description}`;
  return score;
}
...
function EditableHistoryMatchCard({ match, onEditScore, onEditPlayers }: { match: HistoryMatch; onEditScore: (match: HistoryMatch) => void; onEditPlayers: (match: HistoryMatch) => void }) {
  return <HistoryMatchCard match={match} actions={<div className="grid grid-cols-2 gap-2 sm:flex sm:justify-end"><UiButton variant="quiet" className="min-h-11 w-full px-3 sm:w-auto" onClick={() => onEditScore(match)}><Pencil size={15} /> Edit score</UiButton><UiButton variant="quiet" className="min-h-11 w-full px-3 sm:w-auto" onClick={() => onEditPlayers(match)}><UsersRound size={15} /> Edit players</UiButton></div>} />;
}

function HistoryDialogShell({ title, eyebrow, description, children, footer, onClose, pending, size = "narrow" }: { title: string; eyebrow: string; description: string; children: ReactNode; footer: ReactNode; onClose: () => void; pending?: boolean; size?: "narrow" | "wide" }) {
  const dialogRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => { onCloseRef.current = onClose; pendingRef.current = Boolean(pending); }, [onClose, pending]);
  useEffect(() => {
  ...
  const widthClass = size === "wide" ? "sm:max-w-2xl" : "sm:max-w-lg";
  return <div className="fixed inset-0 z-50 grid items-end bg-[#102a2d]/45 p-0 sm:items-center sm:p-5"><div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} className={`flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl outline-none sm:mx-auto sm:rounded-3xl ${widthClass}`}><header className="flex shrink-0 items-start justify-between gap-4 border-b border-[var(--line)] px-5 py-4 sm:px-7 sm:py-5"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--teal)]">{eyebrow}</p><h2 id={titleId} className="display mt-1 text-3xl">{title}</h2></div><UiButton variant="quiet" className="min-h-11 min-w-11 shrink-0 px-3" aria-label="Close correction dialog" disabled={pending} onClick={onClose}><X size={18} /></UiButton></header><div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-7 sm:py-6"><p id={descriptionId} className="sr-only">{description}</p>{children}</div><footer className="shrink-0 border-t border-[var(--line)] bg-white px-5 py-4 sm:px-7">{footer}</footer></div></div>;
}
...
function HistoryScoreCorrectionDialog({ match, pending, error, onConfirm, onClose }: { match: HistoryMatch; pending: boolean; error?: unknown; onConfirm: (games: { teamAScore: number; teamBScore: number }[], reason?: string) => void; onClose: () => void }) {
  ...
  return <HistoryDialogShell title="Correct the score" eyebrow="Match history" description="Review and update the stored score for this completed match." onClose={onClose} pending={pending} footer={<div className="grid gap-2 sm:flex sm:justify-end"><UiButton variant="quiet" type="button" className="min-h-11 w-full sm:w-auto" onClick={onClose} disabled={pending}>Cancel</UiButton><UiButton type="button" className="min-h-11 w-full sm:w-auto" onClick={submit} disabled={pending}>{pending ? "Saving…" : "Save score correction"}</UiButton></div>}><div className="rounded-2xl bg-[#edf8f4] p-4"><div className="flex items-center justify-between gap-3 text-sm font-semibold"><span className="min-w-0 break-words">{teamA}</span><span className="shrink-0 text-xs text-[var(--muted)]">vs</span><span className="min-w-0 break-words text-right">{teamB}</span></div><p className="mt-2 text-xs leading-5 text-[var(--muted)]">Stored rules: race to {settings.pointsToWin}, win by {settings.winBy}, maximum {settings.scoreCap ?? settings.pointsToWin}, best of {settings.bestOf}</p></div><div className="mt-5 space-y-3">{games.map((game, index) => <div key={index} className="rounded-2xl border border-[var(--line)] p-3"><div className="mb-2 flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted)]">Game {index + 1}</span>{settings.bestOf === 3 && <span className="text-xs text-[var(--muted)]">{index === 0 ? "Required" : "Optional"}</span>}</div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><Input className="min-h-11" inputMode="numeric" aria-label={`${teamA} game ${index + 1} score`} value={game.teamAScore} onChange={(event) => updateGame(index, "teamAScore", event.target.value)} /><span className="text-sm text-[var(--muted)]">–</span><Input className="min-h-11" inputMode="numeric" aria-label={`${teamB} game ${index + 1} score`} value={game.teamBScore} onChange={(event) => updateGame(index, "teamBScore", event.target.value)} /></div></div>)}</div>{settings.bestOf === 3 && games.length < 3 && <UiButton type="button" variant="quiet" className="mt-3 min-h-11 w-full" onClick={() => setGames((current) => [...current, { teamAScore: "", teamBScore: "" }])}><Plus size={16} /> Add game</UiButton>}<label className="mt-4 block text-sm font-semibold">Correction reason <span className="font-normal text-[var(--muted)]">(optional)</span><Input className="min-h-11" value={reason} maxLength={240} onChange={(event) => setReason(event.target.value)} placeholder="Score correction" /></label>{(validationError || Boolean(error)) && <p role="alert" className="mt-3 rounded-2xl bg-[#fff0e4] px-4 py-3 text-sm text-[#8d4824]">{validationError || (error ? errorMessage(error) : "")}</p>}</HistoryDialogShell>;
}
...
function HistoryParticipantCorrectionDialog({ match, players, pending, error, onConfirm, onClose }: { match: HistoryMatch; players: SessionPlayer[]; pending: boolean; error?: unknown; onConfirm: (teamA: string[], teamB: string[], reason?: string) => void; onClose: () => void }) {
  ...
  return <HistoryDialogShell title="Correct the players" eyebrow="Match history" description="Review and update the players assigned to each side of this completed match." size="wide" onClose={onClose} pending={pending} footer={<div className="grid gap-2 sm:flex sm:justify-end"><UiButton variant="quiet" type="button" className="min-h-11 w-full sm:w-auto" onClick={onClose} disabled={pending}>Cancel</UiButton><UiButton type="button" className="min-h-11 w-full sm:w-auto" onClick={submit} disabled={pending}>{pending ? "Saving…" : "Save player correction"}</UiButton></div>}><p className="text-sm leading-6 text-[var(--muted)]">Keep the original {initialA.length === 1 ? "singles" : "doubles"} format. Players can be replaced, swapped, or moved between sides using the current queue roster.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="space-y-3 rounded-2xl bg-[#edf8f4] p-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--teal)]">Team A</p>{teamA.map((id, index) => <Fragment key={`a-${index}`}>{slot("A", index, id)}</Fragment>)}</div><div className="space-y-3 rounded-2xl bg-[#fff7f0] p-3"><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#a85b2b]">Team B</p>{teamB.map((id, index) => <Fragment key={`b-${index}`}>{slot("B", index, id)}</Fragment>)}</div></div><label className="mt-4 block text-sm font-semibold">Correction reason <span className="font-normal text-[var(--muted)]">(optional)</span><Input className="min-h-11" value={reason} maxLength={240} onChange={(event) => setReason(event.target.value)} placeholder="Participant correction" /></label>{(validationError || Boolean(error)) && <p role="alert" className="mt-3 rounded-2xl bg-[#fff0e4] px-4 py-3 text-sm text-[#8d4824]">{validationError || (error ? errorMessage(error) : "")}</p>}</HistoryDialogShell>;
}
...
function FeesView({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    ...
    if (selectedPlayerId && !payablePlayers.some((player) => feePlayerId(player) === selectedPlayerId)) {
      setSelectedPlayerId("");
      setPaymentAmount("");
    }
  }, [fee, payablePlayers, selectedPlayerId]);
  /* eslint-enable react-hooks/set-state-in-effect */
  const paymentPlayers = useMemo(() => payablePlayers.map((player) => ({ id: feePlayerId(player), displayName: player.displayName, outstandingMinor: player.outstandingMinor })), [payablePlayers]);
...
function RankingsSharingControls({ session }: { session: SessionSummary }) {
  ...
  const refreshAfterPublishError = () => { void queryClient.invalidateQueries({ queryKey: ["workspace"] }); void queryClient.invalidateQueries({ queryKey: ["rankings"] }); void queryClient.invalidateQueries({ queryKey: ["publicRankingPublications"] }); void queryClient.invalidateQueries({ queryKey: ["rankingTracking"] }); };
  const publish = useMutation({
    ...
    onSuccess: (publication) => { setOptimisticPublication(publication); setPublicSharingActive(true); queryClient.setQueryData<PublicRankingPublicationResponse>(["publicRankingPublications"], (previous) => publishedPublicRankingState(previous, publication)); window.dispatchEvent(new Event("shuttle-queue-public-sharing")); void queryClient.invalidateQueries({ queryKey: ["publicRankingPublications"] }); void queryClient.invalidateQueries({ queryKey: ["rankingTracking"] }); void queryClient.invalidateQueries({ queryKey: ["workspace"] }); toast.success("Public rankings link is ready."); },
  ...
  const revoke = useMutation({
    ...
    onSuccess: (publication) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["publicRankingPublications"] }); void queryClient.invalidateQueries({ queryKey: ["rankingTracking"] });
...
function QueueWorkspaceShell({ user, onLogout }: { user: AuthUser; onLogout: () => void }) {
  ...
  const [tab, setTab] = useState<Tab>(() => typeof window !== "undefined" && new URLSearchParams(window.location.search).get("tab") === "rankings" ? "rankings" : "live");
