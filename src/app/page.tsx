...
function QueuedMatchEditDialog({ match, players, pending, error, onConfirm, onClose }: { match: Match; players: SessionPlayer[]; pending: boolean; error?: unknown; onConfirm: (teamA: string[], teamB: string[]) => void; onClose: () => void }) {
  ...
  const replace = (team: "A" | "B", index: number, value: string) => (team === "A" ? setTeamA : setTeamB)((current) => current.map((id, slot) => slot === index ? value : id));
  const swapTeams = () => { setTeamA(teamB); setTeamB(teamA); };
  const valid = teamA.length === teamB.length && [1, 2].includes(teamA.length) && new Set([...teamA, ...teamB]).size === teamA.length + teamB.length;
...
function LiveMatchEditDialog({ match, players, courts, pending, error, onConfirm, onClose }: { match: Match; players: SessionPlayer[]; courts: Court[]; pending: boolean; error?: unknown; onConfirm: (teamA: string[], teamB: string[], courtId?: string) => void; onClose: () => void }) {
  const [teamA, setTeamA] = useState<string[]>(() => match.participants.filter((participant) => participant.team === "A" && participant.queuePlayerId).sort((a, b) => (a.teamSlot ?? 0) - (b.teamSlot ?? 0)).map((participant) => participant.queuePlayerId!));
  const [teamB, setTeamB] = useState<string[]>(() => match.participants.filter((participant) => participant.team === "B" && participant.queuePlayerId).sort((a, b) => (a.teamSlot ?? 0) - (b.teamSlot ?? 0)).map((participant) => participant.queuePlayerId!));
  ...
  const courtOptions = courts.filter((court) => court.id === match.courtId || court.status === "AVAILABLE");
  const selected = new Set([...teamA, ...teamB]);
  const replace = (team: "A" | "B", index: number, value: string) => (team === "A" ? setTeamA : setTeamB)((current) => current.map((id, slot) => slot === index ? value : id));
  const swapTeams = () => { setTeamA(teamB); setTeamB(teamA); };
  const valid = teamA.length === teamB.length && [1, 2].includes(teamA.length) && new Set([...teamA, ...teamB]).size === teamA.length + teamB.length;
  ...
  return <div className="fixed inset-0 z-50 grid items-end bg-[#102a2d]/45 p-0 sm:items-center sm:p-5">
    <div role="dialog" aria-modal="true" aria-labelledby="edit-live-match-title" className="max-h-[92vh] w-full overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:mx-auto sm:max-w-lg sm:rounded-3xl">
      ...
      <p className="mt-2 text-sm text-[var(--muted)]">Swap teammates, move players between teams, replace a player, or transfer this match to an available court.</p>
      <div className="mt-5 grid grid-cols-2 gap-3">{([['A', teamA], ['B', teamB]] as const).map(([team, ids]) => <div key={team} className={`rounded-2xl p-3 ${team === "A" ? "bg-[#edf8f4]" : "bg-[#fff4ec]"}`}><p className="text-xs font-bold uppercase tracking-[0.12em]">Team {team}</p><div className="mt-3 space-y-2">{ids.map((id, index) => <label key={`${team}-${index}`} className="block text-xs font-semibold">Slot {index + 1}<Select value={id} onChange={(event) => replace(team, index, event.target.value)}>{eligible.map((player) => <option key={player.id} value={player.id} disabled={selected.has(player.id) && player.id !== id}>{labelFor(player.id)} · {pretty(player.status)}</option>)}</Select></label>)}</div></div>)}</div>
      <label className="mt-4 block text-sm font-semibold">Live court<Select aria-label="Live court" value={courtId} onChange={(event) => setCourtId(event.target.value)}><option value="">Keep current court</option>{courtOptions.map((court) => <option value={court.id} key={court.id}>{court.name}{court.id === match.courtId ? " · Current" : " · Available"}</option>)}</Select></label>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-[1fr_1fr_1.2fr]"><Button variant="quiet" className="w-full" onClick={swapTeams} disabled={pending}>Swap teams</Button><Button variant="quiet" className="w-full" onClick={onClose} disabled={pending}>Cancel</Button><Button className="w-full" onClick={() => onConfirm(teamA, teamB, courtId || undefined)} disabled={!valid || pending} loading={pending}>Save live match</Button></div>
    </div>
  </div>;
}
...
function LiveView({ session, sessionId, onStartSession }: { session: SessionSummary; sessionId: string; onStartSession: () => void }) {
  ...
  const edit = useMutation({ mutationFn: (body: { teamA: string[]; teamB: string[]; courtId?: string }) => { if (!editMatch) throw new Error("Select a match."); return api.updateMatch(editMatch, body); }, onSuccess: (_updated, variables) => { const transferred = editMatch?.status === "IN_PROGRESS" && Boolean(variables.courtId) && variables.courtId !== editMatch.courtId; setEditMatch(null); invalidateOperations(); toast.success(transferred ? "Live match updated and transferred." : editMatch?.status === "IN_PROGRESS" ? "Live lineup updated." : "Queued lineup updated."); } });
  ...
  return <div className="live-view space-y-5"><span className="sr-only">Queued matchups</span>
    ...
    {editMatch?.status === "IN_PROGRESS" && <LiveMatchEditDialog match={editMatch} players={editablePlayers} courts={courts} pending={edit.isPending} error={edit.isError ? edit.error : undefined} onConfirm={(teamA, teamB, courtId) => edit.mutate({ teamA, teamB, ...(courtId ? { courtId } : {}) })} onClose={() => { if (!edit.isPending) { edit.reset(); setEditMatch(null); } }} />}
...
function LateArrivalSettingsCard({ session, sessionId }: { session: SessionSummary; sessionId: string }) {
  ...
  const [customDateTimeOverride, setCustomDateTimeOverrideValue] = useState<string | undefined>(undefined);
  ...
  const saveGrace = useMutation({ mutationFn: () => { const minutes = Number(graceMinutes); if (!Number.isInteger(minutes) || minutes < 1 || minutes > 60) throw new Error("Late-arrival grace must be a whole number from 1 to 60 minutes."); return api.updateSettings({ lateArrivalGraceMinutes: minutes }, settings?.version); }, onSuccess: (updated) => { setGraceMinutesOverride(String(updated.lateArrivalGraceMinutes)); queryClient.setQueryData(["settings"], updated); toast.success(`Late-arrival grace set to ${updated.lateArrivalGraceMinutes} minutes.`); } });
  const updatePolicy = useMutation({ mutationFn: (body: { mode: "SET_NOW" | "START_GRACE" | "APPLY_ACCOUNT_DEFAULT" | "DISABLED" | "SET_CUSTOM"; localDateTime?: string }) => api.updateLateArrivalPolicy(sessionId, body, session.version), onSuccess: (updated, variables) => { setCustomDateTimeOverride(undefined); refresh(updated); const corrected = updated.reclassifiedPlayerCount ?? 0; const graceEndsAt = updated.lateArrivalCutoffAt ? new Intl.DateTimeFormat(undefined, { dateStyle: "medium", timeStyle: "medium", timeZone: settings?.timeZone ?? "Asia/Manila" }).format(new Date(updated.lateArrivalCutoffAt)) : null; toast.success(variables.mode === "START_GRACE" ? `${savedGraceMinutes}-minute check-in window ends ${graceEndsAt ?? "soon"}${corrected ? `; ${corrected} pending tag${corrected === 1 ? "" : "s"} cleared.` : "."}` : `Current queue late-arrival policy updated${corrected ? `; ${corrected} pending tag${corrected === 1 ? "" : "s"} cleared.` : "."}`); } });
  const setCustomDateTimeOverride = (value: string | undefined) => { setCustomDateTimeOverrideValue(value); };
  ...
  return <Card><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--teal)]">Arrival policy</p><h3 className="display mt-1 text-3xl">Late check-in rules.</h3></div><Badge tone={session.lateArrivalCutoffAt ? "orange" : "gray"}>{session.lateArrivalCutoffAt ? "Enabled" : "Disabled"}</Badge></div><p className="mt-3 text-sm leading-6 text-[var(--muted)]">Players are classified by their first check-in. Start a server-timed grace window whose duration is set per Queue Master; repeating it resets the end time. Games-played fairness comes before one-time late priority.</p><div className="mt-5 grid gap-5 lg:grid-cols-2"><div className="rounded-2xl bg-[#f7faf8] p-4"><h4 className="font-semibold">Account default preset</h4><p className="mt-1 text-xs leading-5 text-[var(--muted)]">Reusable only; it is never applied automatically. Account timezone: {settings?.timeZone ?? "Asia/Manila"}.</p><label className="mt-3 block text-sm font-semibold">Grace duration (minutes)<Input type="number" min={1} max={60} value={graceMinutes} onChange={(event) => setGraceMinutesOverride(event.target.value)} /></label><Button variant="quiet" className="mt-3" onClick={() => saveGrace.mutate()} disabled={saveGrace.isPending || settingsQuery.isPending}>{saveGrace.isPending ? "Saving…" : "Save grace duration"}</Button>{saveGrace.isError && <p role="alert" className="mt-2 text-xs text-[#8d4824]">{errorMessage(saveGrace.error)}</p>}<label className="mt-3 block text-sm font-semibold">Default arrival cutoff<Input type="time" value={defaultTime} onChange={(event) => setDefaultTimeOverride(event.target.value)} /></label><div className="mt-3 flex flex-wrap gap-2"><Button onClick={() => saveDefault.mutate(defaultTime || null)} disabled={saveDefault.isPending || settingsQuery.isPending}>{saveDefault.isPending ? "Saving…" : "Save default"}</Button><Button variant="quiet" onClick={() => saveDefault.mutate(null)} disabled={saveDefault.isPending}>Clear</Button><Button variant="quiet" onClick={() => setDefaultTimeOverride(currentClockForTimezone(settings?.timeZone ?? "Asia/Manila"))} disabled={saveDefault.isPending}>Use current time</Button></div>{saveDefault.isError && <p role="alert" className="mt-2 text-xs text-[#8d4824]">{errorMessage(saveDefault.error)}</p>}</div><div className="rounded-2xl bg-[#fffaf5] p-4"><h4 className="font-semibold">This session</h4><p className="mt-1 text-xs leading-6 text-[var(--muted)]">Current cutoff: <span className="font-semibold text-[var(--ink)]">{cutoffLabel}</span> ({settings?.timeZone ?? "Asia/Manila"})</p><div className="mt-3 flex flex-wrap gap-2"><Button variant="quiet" onClick={() => updatePolicy.mutate({ mode: "APPLY_ACCOUNT_DEFAULT" })} disabled={disabled || !settings?.defaultLateArrivalCutoffTime || updatePolicy.isPending}>Apply account default</Button><Button variant="quiet" onClick={() => updatePolicy.mutate({ mode: "START_GRACE" })} disabled={disabled || updatePolicy.isPending || saveGrace.isPending}>Start {savedGraceMinutes}-minute check-in window</Button><Button variant="quiet" onClick={() => updatePolicy.mutate({ mode: "DISABLED" })} disabled={disabled || updatePolicy.isPending}>Disable</Button></div><label className="mt-3 block text-sm font-semibold">Custom cutoff ({settings?.timeZone ?? "Asia/Manila"})<Input type="datetime-local" value={customDateTime} onChange={(event) => setCustomDateTimeOverride(event.target.value)} disabled={disabled} /></label><Button className="mt-3 w-full" onClick={() => updatePolicy.mutate({ mode: "SET_CUSTOM", localDateTime: customDateTime })} disabled={disabled || !customDateTime || updatePolicy.isPending}>{updatePolicy.isPending ? "Updating…" : "Save custom cutoff"}</Button><p className="mt-2 text-xs text-[var(--muted)]">Use the custom field only for a specific cutoff. The grace action applies immediately and shows its exact end time.</p>{disabled && <p className="mt-2 text-xs text-[var(--muted)]">Ended or cancelled sessions cannot change arrival rules.</p>}{updatePolicy.isError && <p role="alert" className="mt-2 text-xs text-[#8d4824]">{errorMessage(updatePolicy.error)}</p>}</div></div></Card>;
}
