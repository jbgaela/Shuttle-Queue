...
import { RankingLocationGate } from "@/components/ranking-location-gate";
import { toast } from "sonner";
import { api, ApiError, type PublicRankingMatch, type PublicRankingPayload, type PublicRankingRow } from "@/lib/api";
...
function PublicRankingPlayerRow({ token, visitKey, onAccessLost, row, historyAvailable, live, expanded, onToggle }: { token: string; visitKey: string; onAccessLost: () => void; row: PublicRankingRow; historyAvailable: boolean; live: boolean; expanded: boolean; onToggle: () => void }) {
  ...
  const historyQuery = useQuery({ queryKey: ["publicRankingPlayerHistory", token, visitKey, row.playerKey], queryFn: () => api.publicRankingPlayerHistory(token, row.playerKey as string, visitKey), enabled: canExpand && expanded, retry: false, refetchInterval: expanded && live ? 8_000 : false });
  useEffect(() => { if (historyQuery.error instanceof ApiError && historyQuery.error.code === "LOCATION_REQUIRED") onAccessLost(); }, [historyQuery.error, onAccessLost]);
...
export default function PublicRankingsPage() {
  ...
  return <RankingLocationGate key={token} token={token}>{(visitKey, onAccessLost) => <AuthorizedPublicRankings token={token} visitKey={visitKey} onAccessLost={onAccessLost} />}</RankingLocationGate>;
}

function AuthorizedPublicRankings({ token, visitKey, onAccessLost }: { token: string; visitKey: string; onAccessLost: () => void }) {
  const [expandedPlayerKey, setExpandedPlayerKey] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const rankingsQuery = useQuery<PublicRankingPayload>({ queryKey: ["publicRankings", token, visitKey], queryFn: () => api.publicRankings(token, visitKey), enabled: Boolean(token), retry: false, refetchInterval: (query) => query.state.data?.state === "LIVE" ? 8_000 : false });
  useEffect(() => { if (rankingsQuery.error instanceof ApiError && rankingsQuery.error.code === "LOCATION_REQUIRED") onAccessLost(); }, [rankingsQuery.error, onAccessLost]);
  ...
  return <main className="min-h-screen bg-[var(--paper)] px-4 py-8 sm:px-6 sm:py-12"><div className="mx-auto max-w-3xl"><header className="mb-7"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-[var(--teal)] text-white"><Activity size={21} /></div><div><p className="text-sm font-bold">Shuttle Queue</p><p className="text-xs text-[var(--muted)]">Public session rankings</p></div><span className="ml-auto"><Badge tone={data.state === "LIVE" ? "teal" : "gray"}>{data.state === "LIVE" ? "Live" : "Final"}</Badge></span></div><h1 className="display mt-7 text-4xl sm:text-5xl">LineDrive Afternoon Queue</h1>{metadata && <p className="mt-2 text-sm text-[var(--muted)]">{metadata}</p>}<div className="mt-4 flex justify-end"><Button variant="quiet" className="px-3" disabled={!data.rankings.length || saving} loading={saving} onClick={() => void save()}><Download size={16} /> {saving ? "Saving…" : "Save to device"}</Button></div></header><Card className="overflow-hidden p-0"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--line)] bg-[#fbfdfb] px-4 py-3 text-xs text-[var(--muted)]"><span className="inline-flex items-center gap-1.5"><Clock3 size={14} /> Last published {formatDate(data.lastUpdatedAt)}</span><span>{data.state === "LIVE" ? "Refreshes automatically" : "Final standings"}</span></div>{data.state === "FINAL" && !data.historyAvailable && <p className="border-b border-[var(--line)] bg-[#fffaf5] px-4 py-3 text-xs leading-5 text-[#8d4824]">Match history is not available for this older shared session.</p>}{data.rankings.length === 0 ? <div className="p-8 text-center text-sm text-[var(--muted)]">No players have joined this queue yet.</div> : <><div>{ranked.map((row) => { const rowKey = row.playerKey ?? `${row.rank}-${row.player}`; return <PublicRankingPlayerRow key={rowKey} token={token} visitKey={visitKey} onAccessLost={onAccessLost} row={row} historyAvailable={data.historyAvailable} live={data.state === "LIVE"} expanded={expandedPlayerKey === rowKey} onToggle={() => setExpandedPlayerKey((current) => current === rowKey ? null : rowKey)} />; })}</div><DidNotPlaySection players={didNotPlay} variant="compact" /></>}</Card><p className="mt-5 text-center text-xs leading-5 text-[var(--muted)]">This read-only page is shared by the Queue Master. It contains ranking statistics only.</p><p className="mt-2 text-center text-xs text-[var(--muted)]">Created by: LindeDrive PH: Jean Benedict Gaela & Jendii</p></div></main>;
}
