...
export default function PublicRankingsPage() {
  ...
  return <main className="min-h-screen bg-[var(--paper)] px-4 py-8 sm:px-6 sm:py-12"><div className="mx-auto max-w-3xl"><header className="mb-7"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-[var(--teal)] text-white"><Activity size={21} /></div><div><p className="text-sm font-bold">Shuttle Queue</p><p className="text-xs text-[var(--muted)]">Public session rankings</p></div><span className="ml-auto"><Badge tone={data.state === "LIVE" ? "teal" : "gray"}>{data.state === "LIVE" ? "Live" : "Final"}</Badge></span></div><h1 className="display mt-7 text-4xl sm:text-5xl">Results that stay useful.</h1><p className="mt-2 text-sm text-[var(--muted)]">Session started {formatDate(data.sessionStartedAt)}{data.sessionEndedAt ? ` · ended ${formatDate(data.sessionEndedAt)}` : ""}</p></header><Card className="overflow-hidden p-0"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--line)] bg-[#fbfdfb] px-4 py-3 text-xs text-[var(--muted)]"><span className="inline-flex items-center gap-1.5"><Clock3 size={14} /> Last published {formatDate(data.lastUpdatedAt)}</span><span>{data.state === "LIVE" ? "Refreshes automatically" : "Final standings"}</span></div>{data.state === "FINAL" && !data.historyAvailable && <p className="border-b border-[var(--line)] bg-[#fffaf5] px-4 py-3 text-xs leading-5 text-[#8d4824]">Match history is not available for this older shared session.</p>}{data.rankings.length === 0 ? <div className="p-8 text-center text-sm text-[var(--muted)]">No players have joined this queue yet.</div> : <div>{data.rankings.map((row) => { const rowKey = row.playerKey ?? `${row.rank}-${row.player}`; return <PublicRankingPlayerRow key={rowKey} token={token} row={row} historyAvailable={data.historyAvailable} live={data.state === "LIVE"} expanded={expandedPlayerKey === rowKey} onToggle={() => setExpandedPlayerKey((current) => current === rowKey ? null : rowKey)} />; })}</div>}</Card><p className="mt-5 text-center text-xs leading-5 text-[var(--muted)]">This read-only page is shared by the Queue Master. It contains ranking statistics only.</p><p className="mt-2 text-center text-xs text-[var(--muted)]">Created by: LindeDrive PH: Jean Benedict Gaela</p></div></main>;
}
