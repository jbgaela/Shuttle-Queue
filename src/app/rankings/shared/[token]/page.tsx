...
function PublicRankingPlayerRow({ token, row, historyAvailable, live, expanded, onToggle }: { token: string; row: PublicRankingRow; historyAvailable: boolean; live: boolean; expanded: boolean; onToggle: () => void }) {
  ...
  const rowContent = <><span className={`grid size-9 shrink-0 place-items-center rounded-full text-sm font-bold ${row.isPrizePosition ? "bg-[#fff0c8] text-[#8d5a16]" : "bg-[#edf8f4] text-[var(--teal)]"}`}>{row.rank}</span><span className="min-w-0 flex-1"><span className="block truncate font-semibold">{row.player}{row.isPrizePosition ? <span className="ml-2 text-xs font-bold uppercase tracking-[0.1em] text-[#a85b2b]">Prize</span> : null}</span><span className="mt-0.5 block text-xs text-[var(--muted)]">{row.matchesPlayed} games · {row.wins}W / {row.losses}L{row.seededDrawUsed ? " · seeded draw tiebreak" : ""}</span></span><span className="text-right"><span className="block font-semibold">{(row.winRateBasisPoints / 100).toFixed(0)}%</span><span className="block text-xs text-[var(--muted)]">win rate · score {(row.rankingScoreBasisPoints ?? 0) / 100}%</span></span>{canExpand && <ChevronDown aria-hidden="true" size={18} className={`shrink-0 text-[var(--muted)] transition-transform duration-300 motion-reduce:transition-none ${expanded ? "rotate-180" : ""}`} />}</>;
...
export default function PublicRankingsPage() {
  ...
  const { ranked, notYetEligible, didNotPlay } = partitionRankingRows(data.rankings);
  const save = async () => {
    if (!data.rankings.length || saving) return;
    setSaving(true);
    try {
      await saveRankingsToDevice(data.rankings);
  ...
  return <main className="min-h-screen bg-[var(--paper)] px-4 py-8 sm:px-6 sm:py-12"><div className="mx-auto max-w-3xl"><header className="mb-7"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-[var(--teal)] text-white"><Activity size={21} /></div><div><p className="text-sm font-bold">Shuttle Queue</p><p className="text-xs text-[var(--muted)]">Public session rankings</p></div><span className="ml-auto"><Badge tone={data.state === "LIVE" ? "teal" : "gray"}>{data.state === "LIVE" ? "Live" : "Final"}</Badge></span></div><h1 className="display mt-7 text-4xl sm:text-5xl">LineDrive Afternoon Queue</h1>{metadata && <p className="mt-2 text-sm text-[var(--muted)]">{metadata}</p>}<div className="mt-4 flex justify-end"><Button variant="quiet" className="px-3" disabled={!data.rankings.length || saving} loading={saving} onClick={() => void save()}><Download size={16} /> {saving ? "Saving…" : "Save to device"}</Button></div></header><Card className="overflow-hidden p-0"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--line)] bg-[#fbfdfb] px-4 py-3 text-xs text-[var(--muted)]"><span className="inline-flex items-center gap-1.5"><Clock3 size={14} /> Last published {formatDate(data.lastUpdatedAt)}</span><span>{data.state === "LIVE" ? "Refreshes automatically" : "Final standings"}</span></div><div className="border-b border-[var(--line)] bg-[#fbfdfb] px-4 py-3 text-xs leading-5 text-[var(--muted)]">Players need 5 completed games to qualify. The top 3 eligible players are prize positions.</div>{data.state === "FINAL" && !data.historyAvailable && <p className="border-b border-[var(--line)] bg-[#fffaf5] px-4 py-3 text-xs leading-5 text-[#8d4824]">Match history is not available for this older shared session.</p>}{data.rankings.length === 0 ? <div className="p-8 text-center text-sm text-[var(--muted)]">No players have joined this queue yet.</div> : <><div>{ranked.map((row) => { const rowKey = row.playerKey ?? `${row.rank}-${row.player}`; return <PublicRankingPlayerRow key={rowKey} token={token} row={row} historyAvailable={data.historyAvailable} live={data.state === "LIVE"} expanded={expandedPlayerKey === rowKey} onToggle={() => setExpandedPlayerKey((current) => current === rowKey ? null : rowKey)} />; })}</div><NotYetEligibleSection players={notYetEligible} /><DidNotPlaySection players={didNotPlay} /></>}</Card><p className="mt-5 text-center text-xs leading-5 text-[var(--muted)]">This read-only page is shared by the Queue Master. It contains ranking statistics only.</p><p className="mt-2 text-center text-xs text-[var(--muted)]">Created by: LindeDrive PH: Jean Benedict Gaela & Jendii</p></div></main>;
}
