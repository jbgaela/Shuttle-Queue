...
Deploy with the frontend directory as the Netlify project root. `netlify/edge-functions/ranking-visitor.ts` handles `/api/v2/public/rankings/*`, signs Netlify's observed visitor IP and approximate location, and forwards to the HTTPS `BACKEND_API_BASE_URL`. Configure `PUBLIC_RANKING_EDGE_SECRET` as a server-only variable of at least 32 characters, identical to Render. Configure `BACKEND_API_BASE_URL` as the Render HTTPS API URL ending in `/api/v2`. Both variables must include Functions scope and the intended deploy context in Netlify's environment-variable settings; values placed only in a configuration file are unavailable to the edge runtime. Never use a `NEXT_PUBLIC_` variable for the secret. Redeploy Netlify after changing either value because edge deployments capture environment values at deploy time. Other API paths continue through the existing Next.js rewrite.

Public links use `/rankings/shared/[token]`. The bare `/rankings/share` URL has no corresponding local token contract. Deploy the edge proxy and location gate together with the backend tracking endpoints and database setup. The Render blueprint enables both public access and server-enforced location access, so configure the shared secret before synchronizing it. After deployment, request an invalid token through Netlify: a `404` confirms the request reached Render, while `503 EDGE_CONFIGURATION_ERROR` identifies missing or invalid edge settings and `503 UPSTREAM_UNAVAILABLE` identifies a failed Render connection. Verify a valid test publication on a deployment preview before production use.
